defmodule Mix.Tasks.Pulsar.Install.Docs do
  ...
  def long_doc do
    ...
    * `--yes` or `-y` - Auto-confirm dependency installation prompts. Also
      skips the file-change diff/confirmation prompt and the uncommitted-git-
      changes warning, so files are overwritten without a chance to review
      them first — commit your working tree before using it on an
