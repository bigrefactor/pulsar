defmodule Pulsar.Components.FlashGroup do
  ...
  ## Long-lived toasts and focus

  Status flashes (info, success) auto-dismiss by default, but error and warning flashes
  (`role="alert"`) stay until dismissed, as does any flash with `auto_dismiss={false}`. While
  a long-lived toast is showing, it can sit on top of an interactive control pinned to the
  same screen edge. When toasts can linger and you have focusable controls at that edge,
  either pick a `position` away from them, or add a matching `scroll-margin` (e.g. `scroll-mb-*`
  for a bottom position) to those controls so they stay in view when focused.
  """
  ...
  defp extract_flash_messages(flash, max_items) when is_map(flash) do
    ...
    |> Enum.reduce(%{}, fn {type, message}, messages ->
      key = comparable_flash_key(type)

      Map.update(messages, key, {flash_priority(type), type, message}, fn {priority, current_type, current_message} ->
        if is_binary(type) do
          {min(priority, flash_priority(type)), type, message}
        else
          {min(priority, flash_priority(type)), current_type, current_message}
        end
      end)
    end)
    |> Map.values()
    |> Enum.sort_by(fn {priority, _type, _message} -> priority end)
    |> Enum.map(fn {_priority, type, message} -> {normalize_flash_key(type), message} end)
    |> Enum.take(max_items)
  end
  ...
  defp comparable_flash_key(type) when is_atom(type) or is_binary(type), do: normalize_flash_key(type)
  defp comparable_flash_key(type), do: type
