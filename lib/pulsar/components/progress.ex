defmodule Pulsar.Components.Progress do
  ...
  defp assign_computed(assigns) do
    determinate = assigns.value != nil
    safe_max = if is_number(assigns.max) and assigns.max > 0, do: assigns.max, else: 100
    clamped = if determinate, do: clamp(assigns.value, 0, safe_max)
    fraction = if determinate, do: clamped / safe_max, else: 0.0
    pct = if determinate, do: round(fraction * 100)
    dashoffset = if determinate, do: Float.round(@ring_circumference * (1 - fraction), 2), else: @ring_circumference

    assigns
    |> assign(:determinate, determinate)
    |> assign(:pct, pct)
    |> assign(:aria_valuenow, if(determinate, do: clamped))
