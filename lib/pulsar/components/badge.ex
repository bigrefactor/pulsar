defmodule Pulsar.Components.Badge do
  ...
  # non-interactive addon content (icons, status dots) is left untouched.
  @addon_hit_classes "inline-flex items-center " <>
  ...
  def badge(assigns) do
    ...
    assigns =
      assigns
      |> assign(:class, class)
      |> assign(:addon_hit_classes, @addon_hit_classes)

    ~H"""
    <span class={@class} {@rest}>
      <span :if={@start_addon != []} class={@addon_hit_classes}>{render_slot(@start_addon)}</span>
      {render_slot(@inner_block)}
      <span :if={@end_addon != []} class={@addon_hit_classes}>{render_slot(@end_addon)}</span>
    </span>
    """
  end
