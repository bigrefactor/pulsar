defmodule Pulsar.Components.Textarea do
  ...
  # Size configuration with min-heights and text sizes
  @size_config %{
    "lg" => %{
      max_height: "max-h-80",
      min_height: "min-h-32",
  ...
  def textarea(assigns) do
    ...
    # Normalize field props and compute attributes
    assigns =
      assigns
    ...
    is_invalid = if is_nil(user_invalid), do: has_errors, else: user_invalid
    ...
    <div class="space-y-2">
      <textarea
        ...
        readonly={@readonly}
        class={@class}
        {if @min_height || @max_height, do: [style: build_custom_height_styles(@min_height, @max_height)], else: []}
  ...
  # Get character count display color based on state - using map-based config
  defp get_character_count_color_class(remaining, over_limit, color, max_length) do
    ...
    cond do
      over_limit || (remaining && remaining == 0) ->
        @char_count_colors["danger"]

      remaining && remaining <= warning_threshold ->
        @char_count_colors["warning"]

      true ->
        @char_count_colors["normal"][color] || @char_count_colors["normal"]["neutral"]
    end
  end
