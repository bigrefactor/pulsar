defmodule Pulsar.Components.Sidebar do
  ...
  """
  # Two explicit arities instead of `js \\ %JS{}`: the one-arg form lets
  # `JS.dispatch/2` build the empty pipeline *inside* the JS module, so we never
  # construct the opaque `JS.t()` here (which would trip `call_without_opaque`).
