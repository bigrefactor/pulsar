defmodule Pulsar.Components.Sidebar do
  ...
  def sidebar(assigns) do
    ...
    ~H"""
    <div id={"#{@id}-root"} phx-hook=".PulsarSidebar" data-sidebar-root>
      <nav
        id={@id}
        data-sidebar-panel
        tabindex="-1"
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarSidebar">
      ...
      export default {
        mounted() {
          ...
          this.state = this.panel.dataset.state === "collapsed" ? "collapsed" : "expanded"
        ...
        runCallback(name) {
          const encoded = this.panel.dataset[name]
          if (encoded && encoded !== "[]" && this.liveSocket) {
            this.liveSocket.execJS(this.panel, encoded)
          }
        },
