defmodule Pulsar.Components.Sidebar do
  ...
  # The nav remains the public layout root and direct flex item. It owns width,
  # collapse state, caller classes/rest attributes, and the named group/peer
  # contract. On mobile it is a transparent fixed containing root; the visual
  # panel below moves off-canvas independently so its fixed backdrop stays
  # viewport-relative.
  @root_base_classes "group/sidebar peer/sidebar " <>
                       "shrink-0 h-svh fixed inset-y-0 z-modal pointer-events-none focus-visible:outline-none " <>
                       "lg:static lg:z-auto lg:pointer-events-auto " <>
                       "lg:transition-[width] lg:duration-normal lg:ease-emphasized"

  @panel_base_classes "relative z-modal flex h-full w-full flex-col overflow-hidden pointer-events-auto " <>
  ...
  @collapse_classes "data-[collapsible=icon]:data-[state=collapsed]:lg:w-16 " <>
                      "data-[collapsible=icon]:data-[state=collapsed]:lg:basis-16 " <>
                      "data-[collapsible=offcanvas]:data-[state=collapsed]:lg:w-0 " <>
                      "data-[collapsible=offcanvas]:data-[state=collapsed]:lg:basis-0"

  @panel_collapse_classes "data-[collapsible=offcanvas]:data-[state=collapsed]:lg:border-0 " <>
  ...
  @backdrop_classes [
    ...
    "data-[mobile=open]:opacity-100",
    "data-[mobile=open]:duration-normal data-[mobile=open]:ease-decelerate",
    "data-[mobile=open]:pointer-events-auto"
  ]
  ...
  attr(:class, :string, default: "", doc: "Additional CSS classes for the direct nav/layout root")

  attr(:panel_class, :string,
    default: "",
    doc: "Additional CSS classes for the visual panel (merged last)"
  )

  attr(:rest, :global, doc: "Additional HTML attributes for the direct nav/layout root")
  ...
  def sidebar(assigns) do
    assigns =
      assign(
        ...
        merge([
          root_base_classes(),
          collapse_classes(),
          root_side_classes(assigns.side),
    ...
    assigns =
      assign(
        ...
        merge([
          panel_base_classes(),
          panel_collapse_classes(),
          panel_side_classes(assigns.side),
    ...
    >
      ...
      <div
        ...
        data-mobile="closed"
        aria-hidden="true"
        phx-click={JS.dispatch("pulsar:sidebar-hide")}
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarSidebar">
      ...
      export default {
        mounted() {
          ...
          this.panel = this.el.querySelector("[data-sidebar-panel]")
          this.backdrop = this.el.querySelector(":scope > [data-sidebar-backdrop]")
        ...
        setState(value) {
          ...
          this.panel.dataset.state = value
        ...
        openDrawer() {
          ...
          this.panel.dataset.mobile = "open"
          this.backdrop.dataset.mobile = "open"
          this.lockScroll()
          this.focusFirst()
        ...
        closeDrawer() {
          ...
          this.panel.dataset.mobile = "closed"
          this.backdrop.dataset.mobile = "closed"
          this.unlockScroll()
        ...
        focusableItems() {
          return Array.from(this.panel.querySelectorAll(FOCUSABLE)).filter((el) => el.offsetParent !== null)
        },
        ...
        updated() {
          ...
          this.panel.dataset.mobile = this.mobileOpen ? "open" : "closed"
          this.backdrop.dataset.mobile = this.mobileOpen ? "open" : "closed"
        },
  ...
  """
  def toggle(id), do: JS.dispatch("pulsar:sidebar-toggle", to: "##{id}")

  @doc """
  Toggles the sidebar, composing onto an existing `Phoenix.LiveView.JS` pipeline.
  """
  def toggle(js, id), do: JS.dispatch(js, "pulsar:sidebar-toggle", to: "##{id}")

  @doc """
  Opens (mobile) or expands (desktop) the sidebar. Pass the panel `id`.
  """
  def show(id), do: JS.dispatch("pulsar:sidebar-show", to: "##{id}")

  @doc """
  Opens/expands the sidebar, composing onto an existing `Phoenix.LiveView.JS` pipeline.
  """
  def show(js, id), do: JS.dispatch(js, "pulsar:sidebar-show", to: "##{id}")

  @doc """
  Closes (mobile) or collapses (desktop) the sidebar. Pass the panel `id`.
  """
  def hide(id), do: JS.dispatch("pulsar:sidebar-hide", to: "##{id}")

  @doc """
  Closes/collapses the sidebar, composing onto an existing `Phoenix.LiveView.JS` pipeline.
  """
  def hide(js, id), do: JS.dispatch(js, "pulsar:sidebar-hide", to: "##{id}")

  # ============================================================================
  # HELPER FUNCTIONS
  # ============================================================================

  @spec root_base_classes() :: String.t()
  defp root_base_classes, do: @root_base_classes
  ...
  @spec panel_collapse_classes() :: String.t()
  defp panel_collapse_classes, do: @panel_collapse_classes

  @spec backdrop_classes() :: [String.t()]
  defp backdrop_classes, do: @backdrop_classes

  # Root anchoring stays fixed while the visual panel moves, which keeps the
  # fixed backdrop relative to the viewport rather than a transformed ancestor.
  @spec root_side_classes(String.t()) :: String.t()
  defp root_side_classes("right"), do: "right-0"
  defp root_side_classes(_left), do: "left-0"

  @spec panel_side_classes(String.t()) :: String.t()
  defp panel_side_classes("right") do
    "border-l data-[mobile=closed]:translate-x-full lg:data-[mobile=closed]:translate-x-0"
  end

  defp panel_side_classes(_left) do
    "border-r data-[mobile=closed]:-translate-x-full lg:data-[mobile=closed]:translate-x-0"
  end
