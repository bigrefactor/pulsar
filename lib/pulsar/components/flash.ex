defmodule Pulsar.Components.Flash do
  ...
  # WCAG 2.5.8; padding scales the inner SVG so the X glyph grows with the flash.
  @size_config %{
    "lg" => %{
      close_button: "h-6 w-6 p-0.5",
    ...
    "md" => %{
      ...
      container: "p-3 text-base gap-2 rounded-field",
      icon: "h-5 w-5",
      inner_gap: "gap-2"
    },
    "sm" => %{
      close_button: "h-6 w-6 p-1.5",
  ...
  attr(:auto_dismiss, :boolean,
    default: nil,
    doc:
      ~s{Automatically dismiss after timeout. Defaults to true for role="status" and false for role="alert" so urgent messages don't disappear before users can read them (WCAG 2.2.1).}
  )
  ...
  def flash(assigns) do
    ...
    <div
      ...
      {@rest}
    >
      <div class={["flex items-center flex-1 min-w-0", inner_gap_classes(@size)]}>
        <div :if={@start_icon != []} class={["shrink-0", icon_size_classes(@size)]}>
      ...
      <button
        ...
        aria-controls={@id}
        phx-click={JS.dispatch("pulsar:flash-dismiss")}
      >
        <svg class="h-full w-full" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarFlash">
      // Exit timing — mirrors the `fast` (120ms) duration token. The accelerate
      // easing uses the global `--ease-accelerate` theme token (applied in animateExit).
      const EXIT_MS = 120
      ...
      export default {
        ...
        animateExit() {
          this.el.style.transition = `opacity ${EXIT_MS}ms var(--ease-accelerate), transform ${EXIT_MS}ms var(--ease-accelerate)`
  ...
  @spec base_flash_classes() :: String.t()
  ...
  @spec icon_size_classes(String.t()) :: String.t()
  ...
  defp close_button_classes(size) do
    merge([
      "flex-shrink-0 inline-flex items-center justify-center rounded-field transition-colors",
  ...
  # Inner content wrapper gap based on flash size (matches container gap)
  @spec inner_gap_classes(String.t()) :: String.t()
  defp inner_gap_classes(size) do
    @size_config[size][:inner_gap] || ""
  end

  # Resolve auto_dismiss: nil means "use the role-aware default"
  defp resolve_auto_dismiss(nil, "alert"), do: false
  defp resolve_auto_dismiss(nil, _role), do: true
  defp resolve_auto_dismiss(value, _role) when is_boolean(value), do: value
