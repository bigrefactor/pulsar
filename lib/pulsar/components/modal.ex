defmodule Pulsar.Components.Modal do
  ...
  # A top-layer dialog still inherits text styling from its DOM parent, so the
  # panel states its own color, alignment and cursor. The entrance animation is
  # extracted to `panel_animation` so wrappers such as Drawer can substitute a
  # directional slide.
  @panel_base_classes "z-modal m-auto w-[calc(100%-2rem)] max-h-[85vh] overflow-y-auto " <>
                        "rounded-box text-foreground text-start cursor-auto focus:outline-none " <>
