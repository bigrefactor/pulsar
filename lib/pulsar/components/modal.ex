defmodule Pulsar.Components.Modal do
  ...
  def modal(assigns) do
    ...
    <dialog
      ...
      {@rest}
    >
      <div :if={@has_title || @dismissable} class="mb-4 flex items-start justify-between gap-4">
      ...
      <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarModal">
        export default {
          ...
          handleClick(e) {
            ...
            // the dialog's box; a click on content lands inside it.
            const r = this.el.getBoundingClientRect()
            const outside =
              e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom
            if (e.target === this.el && outside) this.el.close()
          },
