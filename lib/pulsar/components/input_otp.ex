defmodule Pulsar.Components.InputOtp do
  ...
  `on_complete` is a `%JS{}` command run once every character is entered — pair it
  with `JS.push(...)` to auto-submit a verification form.
  """
  ...
  def input_otp(assigns) do
    ...
    <div
      ...
      class={merge(["relative inline-flex items-center", @gap_class, @class])}
    >
      <input
        type="text"
        id={@id}
      ...
      <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarInputOtp">
        export default {
          ...
          boot() {
            ...
            this.slots = Array.from(this.el.querySelectorAll("[data-slot]"));
          ...
          paint() {
            ...
            const activeIndex = focused ? Math.min(v.length, this.length - 1) : -1;
            this.slots.forEach((slot, i) => {
              const ch = v[i] || "";
              const charEl = slot.querySelector("[data-otp-char]");
              const caretEl = slot.querySelector("[data-otp-caret]");
              if (charEl) charEl.textContent = ch ? (this.mask ? "•" : ch) : "";
              slot.setAttribute("data-filled", ch ? "true" : "false");
              const active = i === activeIndex;
              slot.setAttribute("data-active", active ? "true" : "false");
              if (caretEl) caretEl.classList.toggle("hidden", !(active && !ch));
            });
          },
          maybeComplete(v) {
  ...
  defp build_cells(length, nil), do: for(i <- 0..(length - 1), do: {:slot, i})
