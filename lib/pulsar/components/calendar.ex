      ...
      <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarCalendar">
        export default {
          ...
          navButton(label, dir, visible, ariaLabel) {
            ...
            b.className = "flex h-7 w-7 items-center justify-center rounded-field text-muted-foreground hover:bg-surface-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            b.textContent = label
          ...
          attachEvents() {
            ...
            this._onClick = (e) => {
              ...
              if (nav) { this.navigate(nav.dataset.calNav === "next" ? 1 : -1); return }
          ...
          navigate(deltaMonths) {
            this.view = this.addMonths(this.view, deltaMonths)
            this.render()
            // The nav button keeps focus, so announce the new month for screen
            // readers (day cells announce themselves on focus during keyboard nav).
          ...
          moveToWeekEdge(edge) {
            ...
            this.setCursor(this.addDays(this.cursor, delta))
          },
