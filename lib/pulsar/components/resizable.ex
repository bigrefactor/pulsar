defmodule Pulsar.Components.Resizable do
  ...
  ## Examples
      ...
      # Stacked; either panel collapsible
  ...
  `default_size`. A collapsible panel adds a focusable toggle button to the handle — Tab to it and
  press Enter or Space to collapse or expand that panel. Dragging the handle to a
  panel's edge also collapses it.

  ## Without JavaScript

  The component's hook applies `default_size` on mount. In a render where the
  hook never runs (a static/dead render, or JavaScript blocked or failed), the
  second panel shows the fixed 30% fallback regardless of `default_size`.
  """
  ...
  slot :panel,
    required: true,
    doc: "Exactly two panels, in source order. The second panel's `label` names the resize separator." do
    attr :label, :string, doc: "Names this panel's resize separator (second panel) and its collapse toggle button."
    attr :class, :string, doc: "Additional classes for this panel's container."

    attr :collapsible, :boolean,
      doc: "Allow this panel to collapse — drag the handle to this panel's edge or use its toggle button."

    attr :collapsed_size, :integer,
      doc: "This panel's residual size when collapsed, as a percent of the group. Default 0."
  end
      ...
      <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarResizable">
        export default {
          ...
          applySize(pct, animate) {
            ...
            // aria-valuenow tracks the end panel's %. When a panel is collapsed it
            // may fall outside [min,max] (0 end-collapsed, up to 100 start-collapsed)
            // — the APG window-splitter pattern permits this in the collapsed state.
  ...
  defp toggle_button_classes(orientation, side) do
    merge([
      ...
      "transition-[color,rotate] duration-fast ease-standard hover:text-foreground",
