defmodule Pulsar.Components.Table do
  ...
  ## Examples
      ...
      <.table id="users" rows={@users} aria_label="Users">
        <:col
          ...
          on_sort={JS.patch(@name_sort_path)}
        >
          <%= user.name %>
        </:col>
      </.table>

  Sortable columns render their label as a native button, including focus and
  ...
  # When sticky_header is enabled, focusable rows need scroll-margin-top to
  # avoid being obscured by the sticky thead during keyboard navigation
  # (WCAG 2.4.11 Focus Not Obscured). Values mirror the per-size thead
  # row height (vertical padding × 2 + text line-height).
  @sticky_row_scroll_margin %{
  ...
  * an `aria_labelledby` attr — referencing an existing heading's id.
  ...
  def table(assigns) do
    ...
    Enum.each(assigns.col, &validate_sortable_column!/1)
    ...
    <div class={@container_classes} data-reflow-allowed>
      ...
      <div role="status" aria-live="polite" class="sr-only">{if @loading, do: "Loading rows"}</div>
      <table
        {@rest}
      ...
      >
          ...
          <tr>
            ...
            >
              <button
                ...
                id={"#{@id}-sort-#{index}"}
                type="button"
                phx-click={col[:on_sort]}
                class={build_sort_button_classes(col[:align])}
              >
                <span>{col[:label]}</span>
                <Icon.icon
              ...
              <span :if={!Map.get(col, :sortable, false)}>{col[:label]}</span>
            </th>
        ...
        <tbody :if={@loading} id={"#{@id}-loading"} class={@tbody_classes}>
          <tr :for={_ <- 1..5} class={@row_classes}>
            <td :for={_col <- @col} class={build_data_cell_classes(@size, "left", "")}>
              <div class="animate-pulse-subtle bg-surface-1 h-4 rounded"></div>
            </td>
            <td :if={@action != []} class={build_data_cell_classes(@size, "right", "w-0")}>
              <div class="animate-pulse-subtle bg-surface-1 h-6 w-16 rounded"></div>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  defp sort_button_justify_class("center"), do: "justify-center"
  defp sort_button_justify_class("right"), do: "justify-end"
  defp sort_button_justify_class(_align), do: "justify-start"
  ...
  defp build_container_classes(assigns) do
    [
      ...
      assigns.sticky_header &&
        "[&_thead_th]:sticky [&_thead_th]:top-0 [&_thead_th]:z-docked #{@sticky_row_scroll_margin[assigns.size]}",
