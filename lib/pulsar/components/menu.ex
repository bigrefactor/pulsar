defmodule Pulsar.Components.Menu do
  ...
  # Disclosure chevron (vertical): rotates when the in-place group is expanded.
  @chevron_classes "ml-auto shrink-0 transition-transform duration-fast ease-standard group-data-[expanded]/disclosure:rotate-180 group-data-[state=collapsed]/sidebar:lg:hidden"

  # Disclosure chevron (horizontal): rotates while the dropdown is open. The
  # trigger publishes `aria-expanded` (kept in sync by the Popover), so the
  # chevron keys off the trigger group rather than a data-attr.
  @chevron_horizontal "ml-1 shrink-0 transition-transform duration-fast ease-standard group-aria-expanded/trigger:rotate-180"

  # Group panel wrapper (vertical): an in-flow height disclosure. The horizontal
  # group renders its dropdown through the Popover primitive instead.
  @disclosure_wrapper [
  ...
  # Group panel list (vertical). Hidden from focus order until expanded; indents
  # under the parent.
  @disclosure_list [
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarMenu">
      export default {
        mounted() {
          ...
          this._onPanelToggle = (e) => this.handlePanelToggle(e)
          this.el.addEventListener("click", this._onClick)
          this.el.addEventListener("keydown", this._onKeydown)
          // Horizontal group dropdowns are native `[popover]` panels (the Popover
          // primitive). `toggle` doesn't bubble, so listen in the capture phase to
          // bridge their open/close to the menu-level group callbacks.
          this.el.addEventListener("toggle", this._onPanelToggle, true)
        ...
        // back to the server-rendered @open and strips client-added attributes.
        // Re-apply the expand state the user drove on the client so open groups
        // stay open. (Horizontal dropdowns are reconciled by the Popover hook.)
        restoreExpanded() {
        ...
        handleClick(e) {
          // Horizontal dropdowns toggle natively via the trigger's popovertarget;
          // the hook only drives the vertical in-place disclosure.
          if (this.orientation === "horizontal") return
        ...
        // Native popover supplies the horizontal dropdown's open/close, Escape,
        // outside-click dismissal, focus-return, and sibling auto-close. Mirror
        // those state changes onto the menu-level group callbacks.
        handlePanelToggle(e) {
          if (this.orientation !== "horizontal") return
          const panel = e.target
          if (!panel || !panel.matches || !panel.matches("[data-menu-panel]")) return
          this.runCallback(e.newState === "open" ? "onGroupOpen" : "onGroupClose")
        },
        ...
        // Dev aid: a group picks its disclosure type from its own `orientation`,
        // which must match the menu. A mismatch renders the wrong affordance
        // silently, so surface it.
        warnOrientationMismatch() {
          this.el.querySelectorAll("[data-menu-group]").forEach((group) => {
            const isPopover = !!group.querySelector(":scope > [popover]")
            if (this.orientation === "horizontal" && !isPopover) {
              console.warn(`[Menu] group "${group.id}" sits in a horizontal menu but rendered an in-place disclosure — set orientation="horizontal" on it.`)
            } else if (this.orientation === "vertical" && isPopover) {
              console.warn(`[Menu] group "${group.id}" sits in a vertical menu but rendered a dropdown — remove orientation="horizontal" from it.`)
            }
          })
        },
        ...
        destroyed() {
          ...
          this.el.removeEventListener("toggle", this._onPanelToggle, true)
        }
      }
    </script>
    """
  ...
  attr(:orientation, :string,
    default: "vertical",
    values: ~w(vertical horizontal),
    doc: "Set to match the parent menu. vertical renders an in-place disclosure; horizontal renders a dropdown."
  )

  attr(:icon, :string, default: nil, doc: ~s{Leading Heroicon name, e.g. "hero-chart-bar"})
  attr(:open, :boolean, default: false, doc: "Initial expanded state (vertical only)")
  ...
  popover when horizontal. Set `orientation` to match the parent menu.
  ...
  def menu_group(assigns) do
    ...
    <li
      ...
      id={@id}
      class="list-none group-data-[orientation=horizontal]/menu:shrink-0"
      data-menu-group
      {@rest}
    >
      <Popover.popover id={"#{@id}-panel"} variant="elevated" class="p-1 border border-border" data-menu-panel>
        <:trigger>
          <button
            type="button"
            id={"#{@id}-trigger"}
          ...
          >
            ...
            <Icon.icon name="hero-chevron-down" size="xs" class={chevron_horizontal_classes()} />
          </button>
        </:trigger>
    ...
    <li
      :if={@orientation != "horizontal"}
      id={@id}
      class="group/disclosure relative list-none"
      data-menu-group
      data-expanded={(@open && "") || nil}
      {@rest}
    >
      <button
        type="button"
  ...
  @spec chevron_horizontal_classes() :: String.t()
  defp chevron_horizontal_classes, do: @chevron_horizontal
