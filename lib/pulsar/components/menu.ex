defmodule Pulsar.Components.Menu do
  ...
  def menu(assigns) do
    ...
    ~H"""
    <nav :if={@landmark} aria-label={@label}>
      <ul
        id={@id}
        role="list"
        phx-hook=".PulsarMenu"
      ...
      export default {
        ...
        handleKeydown(e) {
          if (e.key === "Escape" && this.orientation === "horizontal") {
            ...
            if (open) {
              e.preventDefault()
              const trigger = open.querySelector("[data-menu-trigger]")
              this.closeGroup(open, trigger)
  ...
  def menu_group(assigns) do
    ...
    ~H"""
    <li class="group/disclosure relative list-none" data-menu-group data-expanded={(@open && "") || nil} {@rest}>
      <button
        type="button"
        id={"#{@id}-trigger"}
        data-menu-trigger
