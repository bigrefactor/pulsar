  ...
  def accordion(assigns) do
    ...
    <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarAccordion">
      export default {
        ...
        // Scope to this accordion's own sections: an accordion nested inside a
        // panel has its own hook, so its items/headers belong to it alone.
        owns(el) { return el.closest("[data-accordion]") === this.el },
        items() { return Array.from(this.el.querySelectorAll("[data-accordion-item]")).filter((i) => this.owns(i)) },
        headers() { return Array.from(this.el.querySelectorAll("[data-accordion-header]")).filter((h) => this.owns(h)) },
        ...
        onClick(e) {
          ...
          if (!header || !this.owns(header)) return
