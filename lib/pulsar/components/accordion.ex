      ...
      export default {
        ...
        toggle(header) {
          ...
          if (willOpen && this.single) {
            this.items().forEach((other) => { if (other !== item && other.hasAttribute("data-expanded")) this.applyState(other, false) })
          }
          this.applyState(item, willOpen)
          const encoded = this.el.dataset.onChange
          if (encoded && encoded !== "[]" && this.liveSocket) {
            this.el.setAttribute("phx-value-id", item.id)
            this.el.setAttribute("phx-value-expanded", willOpen ? "true" : "false")
