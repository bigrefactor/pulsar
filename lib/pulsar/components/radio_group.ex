defmodule Pulsar.Components.RadioGroup do
  ...
  defp resolve_id(assigns, nil), do: assigns[:id] || id_from_name(assigns[:name]) || generate_id("radio-group")

  defp resolve_id(assigns, field),
    do: assigns[:id] || field.id || id_from_name(assigns[:name] || field.name) || generate_id("radio-group")
  ...
  defp radio_input_base_classes do
    """
    appearance-none relative cursor-pointer transition-[color,background-color,border-color,box-shadow] duration-fast ease-standard
    rounded-full border-2
    focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2
    disabled:cursor-not-allowed disabled:opacity-disabled
    before:content-[''] before:absolute before:rounded-full
    before:transition-[transform,opacity] before:duration-fast before:ease-standard
  ...
  defp card_base_classes(color, size) do
    [
      ...
      "cursor-pointer transition-[color,background-color,border-color,box-shadow] duration-fast ease-standard",
