defmodule Pulsar.Components.Checkbox do
  ...
  defp resolve_id(assigns, nil), do: assigns[:id] || id_from_name(assigns[:name]) || generate_id("checkbox")

  defp resolve_id(assigns, field),
    do: assigns[:id] || field.id || id_from_name(assigns[:name] || field.name) || generate_id("checkbox")
  ...
  defp render_checkbox(%{card: true} = assigns), do: render_card_checkbox(assigns)
  defp render_checkbox(assigns), do: render_default_checkbox(assigns)
  ...
  # Colocated hook: syncs the native `indeterminate` IDL property from
  # `data-indeterminate`. Required because `indeterminate` is a JS-only
  # property (no HTML attribute) and `aria-checked` is invalid on a native
  # <input type="checkbox"> per the WAI-ARIA spec.
  defp checkbox_hook(assigns) do
    ~H"""
    <script :type={Phoenix.LiveView.ColocatedHook} name=".PulsarCheckbox">
      export default {
        mounted() {
