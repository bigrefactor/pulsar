defmodule Pulsar.CoreComponents do
  ...
  Drop-in replacement for Phoenix core_components simple_form. Routes through
  the Pulsar form wrapper so failed submissions automatically move focus to
  the first invalid field (`aria-invalid="true"`).
  ...
  def simple_form(assigns) do
    ~H"""
    <Form.form :let={f} for={@for} as={@as} {@rest}>
      <div class="space-y-8">
  ...
  def show_flash(js \\ %JS{}, selector) do
    JS.show(js,
      ...
      time: 300,
      transition:
        {"transition-all ease-standard duration-300", "opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95",
  ...
  def hide_flash(js \\ %JS{}, selector) do
    JS.hide(js,
      ...
      time: 200,
      transition:
        {"transition-all ease-standard duration-normal", "opacity-100 translate-y-0 sm:scale-100",
