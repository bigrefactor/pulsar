...
`lib/pulsar/components/calendar.ex`, `calendar/1`. Blank leading cells are `aria-hidden="true"` —
`lib/pulsar/components/calendar.ex`, `calendar/1`. No images or icon SVGs are rendered by
the component.
...
- The outer container declares `role="application" aria-roledescription="calendar"` —
  `lib/pulsar/components/calendar.ex`, `calendar/1`
- Each month grid carries `role="grid"` with an `aria-label` (localized month/year
  string) — `lib/pulsar/components/calendar.ex`, `calendar/1`
...
- Day cells are `<button role="gridcell">` with a full localized date
  `aria-label` from `Intl.DateTimeFormat` — `lib/pulsar/components/calendar.ex`,
  `calendar/1`
...
- `test "renders the grid container wired to the colocated hook"` asserts
  ...
  `test/pulsar/components/calendar_test.exs`

### 1.3.2 Meaningful Sequence (A) — ✓ PASS

**Evidence:** The hook renders month grids in document order (left-to-right,
first month first) — `lib/pulsar/components/calendar.ex`, `calendar/1`. Within each
grid the weekday header row precedes day cells, and day cells are ordered
day 1 … day N — `lib/pulsar/components/calendar.ex`, `calendar/1`. The `aria-hidden`
...
`lib/pulsar/components/calendar.ex`, `calendar/1`, `cell_classes/2`. Today's cell carries
`aria-current="date"` in addition to its bold ring visual —
`lib/pulsar/components/calendar.ex`, `calendar/1`, `cell_classes/2`. Selected state is conveyed via
...
(`lib/pulsar/components/calendar.ex`, `calendar/1`, `cell_classes/2`); selected →
`aria-selected="true"`
(`calendar/1`); today → `aria-current="date"` +
font-semibold ring (`calendar/1`, `cell_classes/2`); in-range →
`data-in-range` attribute (`calendar/1`). Color
...
container — `lib/pulsar/components/calendar.ex`, `cell_classes/2`, `calendar/1`.
Weekday header labels
use `text-muted-foreground` — `lib/pulsar/components/calendar.ex`, `calendar/1`. The
...
use `text-muted-foreground` as well — `lib/pulsar/components/calendar.ex`, `calendar/1`. The
...
— `lib/pulsar/components/calendar.ex`, `cell_classes/2`. Fixed pixel cell dimensions
...
the bounding box at large user-override scales — `lib/pulsar/components/calendar.ex`,
`cell_classes/2`.

### 1.4.10 Reflow (AA) — ✓ PASS

**Evidence:** The outer container is `inline-flex flex-col gap-3` with no enforced
minimum width — `lib/pulsar/components/calendar.ex`, `calendar/1`. For multi-month (`mode="range"`
default 2 months) the grid area is `flex flex-wrap gap-4`
(`lib/pulsar/components/calendar.ex`, `calendar/1`), so secondary month grids wrap to the
...
- Day-cell focus ring uses `focus-visible:ring-2 focus-visible:ring-ring` —
  `lib/pulsar/components/calendar.ex`, `cell_classes/2`. The `--color-ring` token measures
...
- Nav button focus ring uses the same `focus-visible:ring-2 focus-visible:ring-ring`
  token — `lib/pulsar/components/calendar.ex`, `calendar/1`.
- Today's cell carries `ring-1 ring-border-strong` — `lib/pulsar/components/calendar.ex`,
  `cell_classes/2`.
...
user overrides — `lib/pulsar/components/calendar.ex`, `cell_classes/2`.

### 2.1.1 Keyboard (A) — ✓ PASS

**Evidence:** The full APG Date Picker Grid keyboard contract is implemented in the
`onKeydown` handler at `lib/pulsar/components/calendar.ex`, `calendar/1`:
- Arrow keys move by day — `calendar/1`
...
target — `calendar/1`. Navigation across month
...
`map` object at `lib/pulsar/components/calendar.ex`, `calendar/1`. Tab and Shift+Tab are
not in the map and fall through (`if (!fn) return` — `calendar/1`),
...
on day cells — `lib/pulsar/components/calendar.ex`, `cell_classes/2`. No auto-updating or
...
`lib/pulsar/components/calendar.ex`, `cell_classes/2`.

### 2.4.3 Focus Order (A) — ✓ PASS

**Evidence:** Roving tabindex is used: `cell.tabIndex = this.sameDay(date, this.cursor) ? 0 : -1`
— `lib/pulsar/components/calendar.ex`, `calendar/1`. Only the current cursor day cell has
...
consistent after selection — `calendar/1`. The test
`test "ArrowRight moves the focused cell and Enter selects it"` verifies `tabindex="0"` migrates to the new
...
`lib/pulsar/components/calendar.ex`, `calendar/1`. Weekday column headers carry long-form
`aria-label` names — `calendar/1`. Navigation buttons
carry descriptive labels including the destination month —
`calendar/1`. Day cells carry full localized date
...
focus-visible:ring-ring` — `lib/pulsar/components/calendar.ex`, `cell_classes/2`. Navigation
buttons apply the same — `lib/pulsar/components/calendar.ex`, `calendar/1`. The
...
creates no sticky layers — `lib/pulsar/components/calendar.ex`, `calendar/1`.

### 2.5.2 Pointer Cancellation (A) — ✓ PASS

**Evidence:** The click handler is registered via the native `click` event (fires
on mouseup) — `lib/pulsar/components/calendar.ex`, `calendar/1`. No `mousedown` or
...
`aria-label` provides the full name — `lib/pulsar/components/calendar.ex`, `calendar/1`.
Day cells display the day number; the `aria-label` is the full date string that
includes the same number — `calendar/1`.
...
`lib/pulsar/components/calendar.ex`, `cell_classes/2`. Navigation buttons are `h-7 w-7` (28×28 px) —
`lib/pulsar/components/calendar.ex`, `calendar/1`. Adjacent cells are separated by
...
`lib/pulsar/components/calendar.ex`, `calendar/1`. Focusing the calendar container or
...
`on_select` `%JS{}` command — `lib/pulsar/components/calendar.ex`, `calendar/1`. The
component does not trigger page navigation or form submission itself; those actions
are caller-initiated via `JS.push(...)`. The test
`test "selecting a day notifies LiveView via phx-change (the dispatched input event)"`
verifies the side-effect is limited to a controlled event
...
`lib/pulsar/components/calendar.ex`, `calendar/1`. When focus moves to a day cell during
...
- **2.4.13 Focus Appearance (AAA, new in 2.2)** — `ring-2` (2px) meets the AAA
  ...
  6.72:1 dark) — `lib/pulsar/components/calendar.ex`, `cell_classes/2`.
