...
`lib/pulsar/components/calendar.ex:318`. Blank leading cells are `aria-hidden="true"` —
`lib/pulsar/components/calendar.ex:327`. No images or icon SVGs are rendered by
the component.
...
first month first) — `lib/pulsar/components/calendar.ex:259–261`. Within each
grid the weekday header row precedes day cells, and day cells are ordered
day 1 … day N — `lib/pulsar/components/calendar.ex:285–307`. The `aria-hidden`
...
`lib/pulsar/components/calendar.ex:349, 574`. Today's cell carries
`aria-current="date"` in addition to its bold ring visual —
`lib/pulsar/components/calendar.ex:347, 573`. Selected state is conveyed via
`aria-selected="true"` in addition to the accent background —
`lib/pulsar/components/calendar.ex:348`.
...
(`lib/pulsar/components/calendar.ex:349, 574`); selected → `aria-selected="true"`
(`lib/pulsar/components/calendar.ex:348`); today → `aria-current="date"` +
font-semibold ring (`lib/pulsar/components/calendar.ex:347, 573`); in-range →
`data-in-range` attribute (`lib/pulsar/components/calendar.ex:346`). Color
...
container — `lib/pulsar/components/calendar.ex:57–71, 126`. Weekday header labels
use `text-muted-foreground` — `lib/pulsar/components/calendar.ex:293`. The
...
use `text-muted-foreground` as well — `lib/pulsar/components/calendar.ex:319`. The
axe gate is clean across the `/components/calendar/*` fixture cells in both light
and dark themes (134 axe tests passing, Tasks 9).
...
the bounding box at large user-override scales — `lib/pulsar/components/calendar.ex:572`.

### 1.4.10 Reflow (AA) — ✓ PASS

**Evidence:** The outer container is `inline-flex flex-col gap-3` with no enforced
minimum width — `lib/pulsar/components/calendar.ex:126`. For multi-month (`mode="range"`
default 2 months) the grid area is `flex flex-wrap gap-4`
(`lib/pulsar/components/calendar.ex:132`), so secondary month grids wrap to the
...
- Day-cell focus ring uses `focus-visible:ring-2 focus-visible:ring-ring` —
  `lib/pulsar/components/calendar.ex:575`. The `--color-ring` token measures
...
- Nav button focus ring uses the same `focus-visible:ring-2 focus-visible:ring-ring`
  token — `lib/pulsar/components/calendar.ex:319`.
- Today's cell carries `ring-1 ring-border-strong` — `lib/pulsar/components/calendar.ex:573`.
...
user overrides — `lib/pulsar/components/calendar.ex:48–52, 572`.

### 2.1.1 Keyboard (A) — ✓ PASS

**Evidence:** The full APG Date Picker Grid keyboard contract is implemented in the
`onKeydown` handler at `lib/pulsar/components/calendar.ex:453–469`:
- Arrow keys move by day — `lib/pulsar/components/calendar.ex:455–458`
- Home/End move to the first/last day of the current week —
  `lib/pulsar/components/calendar.ex:459–460, 487–490`
- PageUp/PageDown page by one month; Shift+PageUp/Down pages by one year —
  `lib/pulsar/components/calendar.ex:461–462, 493`
- Enter and Space activate the cursor cell —
  `lib/pulsar/components/calendar.ex:463–464, 513–515`

The `moveCursor` helper skips disabled cells when looking for the next tabbable
target — `lib/pulsar/components/calendar.ex:473–484`. Navigation across month
boundaries automatically shifts the visible view and focuses the target cell —
`lib/pulsar/components/calendar.ex:502–510`.
...
**Evidence:** The `onKeydown` handler intercepts only the eight keys listed in the
`map` object at `lib/pulsar/components/calendar.ex:454–465`. Tab and Shift+Tab are
not in the map and fall through (`if (!fn) return` — `lib/pulsar/components/calendar.ex:466`),
...
on day cells — `lib/pulsar/components/calendar.ex:572`. No auto-updating or
...
`lib/pulsar/components/calendar.ex:572`.

### 2.4.3 Focus Order (A) — ✓ PASS

**Evidence:** Roving tabindex is used: `cell.tabIndex = this.sameDay(date, this.cursor) ? 0 : -1`
— `lib/pulsar/components/calendar.ex:352`. Only the current cursor day cell has
...
consistent after selection — `lib/pulsar/components/calendar.ex:400`. The test
...
`lib/pulsar/components/calendar.ex:282`. Weekday column headers carry long-form
`aria-label` names — `lib/pulsar/components/calendar.ex:292`. Navigation buttons
carry descriptive labels including the destination month —
`lib/pulsar/components/calendar.ex:318`. Day cells carry full localized date
`aria-label` strings from `Intl.DateTimeFormat { dateStyle: "full" }` —
`lib/pulsar/components/calendar.ex:340`.

### 2.4.7 Focus Visible (AA) — ✓ PASS

**Evidence:** Day cells apply `focus-visible:outline-none focus-visible:ring-2
focus-visible:ring-ring` — `lib/pulsar/components/calendar.ex:575`. Navigation
buttons apply the same — `lib/pulsar/components/calendar.ex:319`. The
...
creates no sticky layers — `lib/pulsar/components/calendar.ex:110–133`.

### 2.5.2 Pointer Cancellation (A) — ✓ PASS

**Evidence:** The click handler is registered via the native `click` event (fires
on mouseup) — `lib/pulsar/components/calendar.ex:362–367`. No `mousedown` or
...
`aria-label` provides the full name — `lib/pulsar/components/calendar.ex:318, 320`.
Day cells display the day number; the `aria-label` is the full date string that
includes the same number — `lib/pulsar/components/calendar.ex:338, 340`.
...
`lib/pulsar/components/calendar.ex:319`. Adjacent cells are separated by
...
`lib/pulsar/components/calendar.ex:513–515`. Focusing the calendar container or
...
`on_select` `%JS{}` command — `lib/pulsar/components/calendar.ex:382–388`. The
...
- Month grid: `role="grid"` + `aria-label` (localized month/year) —
  `lib/pulsar/components/calendar.ex:281–282`
...
`lib/pulsar/components/calendar.ex:133, 379`. When focus moves to a day cell during
...
- **2.4.13 Focus Appearance (AAA, new in 2.2)** — `ring-2` (2px) meets the AAA
  ...
  6.72:1 dark) — `lib/pulsar/components/calendar.ex:575`.
