...
`lib/pulsar/components/avatar.ex`, `avatar_content/1`. Initials/icon fallbacks expose the
accessible name on the wrapper via `role="img"` + `aria-label={alt || name}`
(`lib/pulsar/components/avatar.ex`, `render_avatar/1`), with the visible initials
(`lib/pulsar/components/avatar.ex`, `avatar_content/1`) and the fallback `<.icon>`
(`lib/pulsar/components/avatar.ex`, `avatar_content/1`, decorative `aria-hidden="true"` by the
...
(`lib/pulsar/components/avatar.ex`, `accessible_name/2`).

### 1.3.1 Info and Relationships (A) — ✓ PASS

**Evidence:** Single inline element per avatar; the group is a flat
`<div>` of avatars plus an optional counter —
`lib/pulsar/components/avatar.ex`, `avatar_group/1`. No implied grouping semantics to
preserve.
...
counter — `lib/pulsar/components/avatar.ex`, `avatar_group/1`. `-space-x-2` overlaps
...
color alone. The muted fallback surface (`lib/pulsar/components/avatar.ex`, `variant_classes/1`)
is decorative.
...
`lib/pulsar/components/avatar.ex`, `variant_classes/1`, `text_classes/1`. Browser measurement of 39
...
(`w-*`/`h-*`/`text-*`) — `lib/pulsar/components/avatar.ex`, `box_classes/1`,
`text_classes/1`. No fixed `px`.
...
constraints — `lib/pulsar/components/avatar.ex`, `build_avatar_classes/1`,
`avatar_group/1`. The group wraps via
...
`bg-background` — `lib/pulsar/components/avatar.ex`, `variant_classes/1`. The link focus ring is
`focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2` —
`lib/pulsar/components/avatar.ex`, `build_avatar_classes/1`. Browser measurement: outline border
...
is token-based — `lib/pulsar/components/avatar.ex`, `box_classes/1`,
`text_classes/1`, `avatar_content/1`.

### 2.1.1 Keyboard (A) — ✓ PASS

**Evidence:** A linked avatar renders through the secure `Link.a` component as a
native `<a>` — `lib/pulsar/components/avatar.ex`, `render_avatar/1`, focusable and
...
(`lib/pulsar/components/avatar.ex`, `render_avatar/1`); focus moves away normally. The
...
`aria-label={alt || name}` — `lib/pulsar/components/avatar.ex`, `render_avatar/1`,
`link_label/2`. A
linked image avatar is named by its `<img alt>` (the link `aria-label` is
suppressed to avoid a double name) — `lib/pulsar/components/avatar.ex`, `link_label/2`. A
...
`lib/pulsar/components/avatar.ex`, `avatar/1`, `warn_if_unnamed_link/1`.
...
by tests `test "warns when a linked avatar has no accessible name"`,
`test "warns when a linked image avatar is explicitly decorative"`,
`test "does not warn when a linked avatar is named"`, and
`test "does not warn for a non-interactive unnamed avatar"` —
`test/pulsar/components/avatar_test.exs`.

### 2.4.7 Focus Visible (AA) — ✓ PASS

**Evidence:** The link variant carries `focus-visible:ring-2
focus-visible:ring-ring focus-visible:ring-offset-2` —
`lib/pulsar/components/avatar.ex`, `build_avatar_classes/1`, `counter_classes/1`. Measured ring contrast 5.02:1
(light) / 4.5:1 (dark).
...
`lib/pulsar/components/avatar.ex`, `avatar_group/1`.
...
(`lib/pulsar/components/avatar.ex`, `render_avatar/1`) that activates on the up-event with
...
(`lib/pulsar/components/avatar.ex`, `render_avatar/1`, `avatar_content/1`). No mismatch between a visible text
label and the accessible name arises.

### 2.5.8 Target Size (Minimum) (AA, new in 2.2) — ✓ PASS

**Evidence:** A linked avatar's hit area equals the avatar box. The smallest
size (`xs`) is `w-6 h-6` = 24×24 CSS px — `lib/pulsar/components/avatar.ex`, `box_classes/1`.
...
**Evidence:** Focusing a linked avatar (`lib/pulsar/components/avatar.ex`, `render_avatar/1`)
...
(`lib/pulsar/components/avatar.ex`, `avatar_content/1`); fallbacks use `role="img"` +
`aria-label` on the wrapper (`lib/pulsar/components/avatar.ex`, `render_avatar/1`); linked
avatars are native `<a>` named via `aria-label` or the nested `<img alt>`
(`lib/pulsar/components/avatar.ex`, `render_avatar/1`). `@rest` forwards `id`/`data-*`/ARIA
(`lib/pulsar/components/avatar.ex`, `avatar/1`, `render_avatar/1`).

**Notes:** Tests confirm role/label resolution and global pass-through —
`test "fallback initials expose the name via role=img + aria-label"`,
`test "the image wrapper does not duplicate the accessible name"`, and
`test "passes through global attributes"` —
`test/pulsar/components/avatar_test.exs`.
