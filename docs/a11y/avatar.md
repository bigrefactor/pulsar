...
`lib/pulsar/components/avatar.ex:196`. Initials/icon fallbacks expose the
accessible name on the wrapper via `role="img"` + `aria-label={alt || name}`
(`lib/pulsar/components/avatar.ex:179`), with the visible initials
(`lib/pulsar/components/avatar.ex:202`) and the fallback `<.icon>`
(`lib/pulsar/components/avatar.ex:208`, decorative `aria-hidden="true"` by the
...
(`lib/pulsar/components/avatar.ex:349`).

### 1.3.1 Info and Relationships (A) — ✓ PASS

**Evidence:** Single inline element per avatar; the group is a flat
`<div>` of avatars plus an optional counter —
`lib/pulsar/components/avatar.ex:262–284`. No implied grouping semantics to
preserve.
...
counter — `lib/pulsar/components/avatar.ex:264–266`. `-space-x-2` overlaps
...
color alone. The muted fallback surface (`lib/pulsar/components/avatar.ex:76`)
is decorative.
...
`lib/pulsar/components/avatar.ex:76–78, 327–328`. Browser measurement of 39
...
(`w-*`/`h-*`/`text-*`) — `lib/pulsar/components/avatar.ex:65–70`. No fixed `px`.
...
constraints — `lib/pulsar/components/avatar.ex:72–73, 85`. The group wraps via
...
`bg-background` — `lib/pulsar/components/avatar.ex:77`. The link focus ring is
`focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2` —
`lib/pulsar/components/avatar.ex:81–82`. Browser measurement: outline border
...
is token-based — `lib/pulsar/components/avatar.ex:65–70, 202`.

### 2.1.1 Keyboard (A) — ✓ PASS

**Evidence:** A linked avatar renders through the secure `Link.a` component as a
native `<a>` — `lib/pulsar/components/avatar.ex:150–172`, focusable and
...
(`lib/pulsar/components/avatar.ex:150–172`); focus moves away normally. The
...
`aria-label={alt || name}` — `lib/pulsar/components/avatar.ex:162, 351`. A
linked image avatar is named by its `<img alt>` (the link `aria-label` is
suppressed to avoid a double name) — `lib/pulsar/components/avatar.ex:350`.

**Notes:** The destination is described by the entity name. Provide `name` (or
`alt`) on linked avatars.

### 2.4.7 Focus Visible (AA) — ✓ PASS

**Evidence:** The link variant carries `focus-visible:ring-2
focus-visible:ring-ring focus-visible:ring-offset-2` —
`lib/pulsar/components/avatar.ex:81–82, 320`. Measured ring contrast 5.02:1
(light) / 4.5:1 (dark).
...
`lib/pulsar/components/avatar.ex:85`.
...
(`lib/pulsar/components/avatar.ex:150–172`) that activates on the up-event with
...
(`lib/pulsar/components/avatar.ex:179, 202`). No mismatch between a visible text
label and the accessible name arises.

### 2.5.8 Target Size (Minimum) (AA, new in 2.2) — ✓ PASS

**Evidence:** A linked avatar's hit area equals the avatar box. The smallest
size (`xs`) is `w-6 h-6` = 24×24 CSS px — `lib/pulsar/components/avatar.ex:65`.
...
**Evidence:** Focusing a linked avatar (`lib/pulsar/components/avatar.ex:150–172`)
...
(`lib/pulsar/components/avatar.ex:196`); fallbacks use `role="img"` +
`aria-label` on the wrapper (`lib/pulsar/components/avatar.ex:179`); linked
avatars are native `<a>` named via `aria-label` or the nested `<img alt>`
(`lib/pulsar/components/avatar.ex:150–172`). `@rest` forwards `id`/`data-*`/ARIA
(`lib/pulsar/components/avatar.ex:118, 179`).

**Notes:** Tests confirm role/label resolution and global pass-through —
`test/pulsar/components/avatar_test.exs:99–133, 201–215`.
