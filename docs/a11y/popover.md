...
`mounted`/`setupClick` wiring in `lib/pulsar/components/popover.ex`, `popover/1`.
The keyboard fixture asserts the expanded state on open/close — `test/integration/a11y/keyboard/popover_test.exs`.
...
`border-{color}` / `border-border-strong` — `lib/pulsar/components/popover.ex`,
`color_classes/2`. These token pairs are the
...
only width floor is `min-w-48` — `lib/pulsar/components/popover.ex`, `popover/1`
(hook `position`) and `base_classes/0` (`@panel_base_classes`).

### 1.4.11 Non-text Contrast (AA) — ✓ PASS

**Evidence:** `elevated` delineates with `shadow-dropdown`; `outline`/`solid` with
a `border-{color}` (2px) — `lib/pulsar/components/popover.ex`, `color_classes/2`.
Same tokens as Card's audited matrix — [`card.md`](card.md).
...
is persistent until dismissed — `lib/pulsar/components/popover.ex`, `popover/1`
(the panel's `popover="auto"` attribute and the hook's `toggle` handling).
...
responsibility) — `lib/pulsar/components/popover.ex`, `base_classes/0`
(`@panel_base_classes`).
...
`lib/pulsar/components/popover.ex`, `popover/1` (hook `position`).
...
on activation — `lib/pulsar/components/popover.ex`, `popover/1`.
...
attributes — `lib/pulsar/components/popover.ex`, `popover/1` (hook
`mounted`/`setupClick`; panel `{@rest}`). Unit tests assert the panel markup and
passthrough — `test/pulsar/components/popover_test.exs`.
