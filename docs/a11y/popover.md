...
`mounted`/`setupClick` wiring in `lib/pulsar/components/popover.ex:252–276`. The keyboard
...
`border-{color}` / `border-border-strong` — `lib/pulsar/components/popover.ex:99–136`.
...
only width floor is `min-w-48` — `lib/pulsar/components/popover.ex:378–434`
(`position`), `:92` (`@panel_base_classes`).

### 1.4.11 Non-text Contrast (AA) — ✓ PASS

**Evidence:** `elevated` delineates with `shadow-dropdown`; `outline`/`solid` with
a `border-{color}` (2px) — `lib/pulsar/components/popover.ex:99–136`. Same tokens as Card's audited matrix — [`card.md`](card.md).
...
is persistent until dismissed — `lib/pulsar/components/popover.ex:236` (the panel's
...
`lib/pulsar/components/popover.ex:270`); Escape closes natively. The keyboard
...
`lib/pulsar/components/popover.ex:234–248`.
...
`lib/pulsar/components/popover.ex:378–434` (`position`).

### 2.5.2 Pointer Cancellation (A) — ✓ PASS

**Evidence:** The trigger activates on `click` (pointer-up); outside-click
dismissal is the browser's native light-dismiss — `lib/pulsar/components/popover.ex:236` (`popover="auto"`).
...
on activation — `lib/pulsar/components/popover.ex:234–248`.
...
attributes — `lib/pulsar/components/popover.ex:252–276` (hook `mounted`/`setupClick`), `:245` (panel
