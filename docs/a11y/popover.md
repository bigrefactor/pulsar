...
`mounted`/`setupClick` wiring in `lib/pulsar/components/popover.ex:258–282`. The keyboard
...
`border-{color}` / `border-border-strong` — `lib/pulsar/components/popover.ex:99–142`.
...
only width floor is `min-w-48` — `lib/pulsar/components/popover.ex:384–440`
(`position`), `:92` (`@panel_base_classes`).

### 1.4.11 Non-text Contrast (AA) — ✓ PASS

**Evidence:** `elevated` delineates with `shadow-dropdown`; `outline`/`solid` with
a `border-{color}` (2px) — `lib/pulsar/components/popover.ex:99–142`. Same tokens as Card's audited matrix — [`card.md`](card.md).
...
is persistent until dismissed — `lib/pulsar/components/popover.ex:242` (the panel's
`popover="auto"` attribute), `:266–276` (the hook's `toggle` handling).
...
`lib/pulsar/components/popover.ex:276`); Escape closes natively. The keyboard
...
`lib/pulsar/components/popover.ex:240–254`.
...
responsibility) — `lib/pulsar/components/popover.ex:92` (`@panel_base_classes`).
...
`lib/pulsar/components/popover.ex:384–440` (`position`).

### 2.5.2 Pointer Cancellation (A) — ✓ PASS

**Evidence:** The trigger activates on `click` (pointer-up); outside-click
dismissal is the browser's native light-dismiss — `lib/pulsar/components/popover.ex:242` (`popover="auto"`).
...
on activation — `lib/pulsar/components/popover.ex:240–254`.
...
attributes — `lib/pulsar/components/popover.ex:258–282` (hook `mounted`/`setupClick`), `:251` (panel
