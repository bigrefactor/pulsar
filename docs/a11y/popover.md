...
only width floor is `min-w-48` — `lib/pulsar/components/popover.ex:405–461`
...
is persistent until dismissed — `lib/pulsar/components/popover.ex:248` (the panel's
...
`lib/pulsar/components/popover.ex:246–261`.
...
`lib/pulsar/components/popover.ex:405–461` (`position`).

### 2.5.2 Pointer Cancellation (A) — ✓ PASS

**Evidence:** The trigger activates on `click` (pointer-up); outside-click
dismissal is the browser's native light-dismiss — `lib/pulsar/components/popover.ex:248` (`popover="auto"`).
...
on activation — `lib/pulsar/components/popover.ex:246–261`.
...
attributes — `lib/pulsar/components/popover.ex:265–303` (hook `mounted`/`setupClick`), `:251` (panel
