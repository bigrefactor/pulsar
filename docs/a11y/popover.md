...
`mounted`/`onToggle` wiring in `lib/pulsar/components/popover.ex:235–277`. The keyboard
...
`border-{color}` / `border-border-strong` — `lib/pulsar/components/popover.ex:90–126`.
...
only width floor is `min-w-48` — `lib/pulsar/components/popover.ex:284–337`
(`position`), `:83` (`@panel_base_classes`).

### 1.4.11 Non-text Contrast (AA) — ✓ PASS

**Evidence:** `elevated` delineates with `shadow-dropdown`; `outline`/`solid` with
a `border-{color}` (2px) — `lib/pulsar/components/popover.ex:90–126`. Same tokens as Card's audited matrix — [`card.md`](card.md).
...
is persistent until dismissed — `lib/pulsar/components/popover.ex:220` (the panel's
`popover="auto"` attribute), `:264–277` (the hook's `toggle` handling).
...
`lib/pulsar/components/popover.ex:240`); Escape closes natively. The keyboard
...
`lib/pulsar/components/popover.ex:218–229`.
...
responsibility) — `lib/pulsar/components/popover.ex:83` (`@panel_base_classes`).
...
`lib/pulsar/components/popover.ex:284–337` (`position`).

### 2.5.2 Pointer Cancellation (A) — ✓ PASS

**Evidence:** The trigger activates on `click` (pointer-up); outside-click
dismissal is the browser's native light-dismiss — `lib/pulsar/components/popover.ex:220` (`popover="auto"`).
...
on activation — `lib/pulsar/components/popover.ex:218–229`.
...
attributes — `lib/pulsar/components/popover.ex:235–277` (hook `mounted`/`onToggle`), `:228` (panel
