...
caller's responsibility — `lib/pulsar/components/input.ex`, `input/1`.

### 1.3.1 Info and Relationships (A) — ✓ PASS

**Evidence:**
- Native `<input>` carries its semantic type via `type={@type}` —
  `lib/pulsar/components/input.ex`, `render_styled_input/2`
- `aria-invalid` reflects validation state from Phoenix form errors or
  the explicit `:invalid` attr — `render_styled_input/2`
- `test "sets aria-invalid to 'true' when field has errors"` —
  `test/pulsar/components/input_test.exs`
...
`lib/pulsar/components/input.ex`, `render_styled_input/2`. Matches
visual order.

### 1.3.3 Sensory Characteristics (A) — ✓ PASS

**Evidence:** Error state combines color (`text-danger`/`bg-danger`),
border, and `aria-invalid="true"` — `lib/pulsar/components/input.ex`,
`get_color_classes/2`, `render_styled_input/2`. Disabled state combines
opacity + cursor + native `disabled` — `render_styled_input/2`,
`get_state_classes/2`.
...
native input — `lib/pulsar/components/input.ex`, `input/1` (attr
declaration), `render_styled_input/2` (forwarded via `{@rest}`).

### 1.4.1 Use of Color (A) — ✓ PASS

**Evidence:** Error state pairs danger color with `aria-invalid="true"`
and (when used via field) a text error message —
`lib/pulsar/components/input.ex`, `get_color_classes/2`,
`render_styled_input/2`. Disabled state combines opacity + cursor +
native `disabled` attr.

### 1.4.3 Contrast (Minimum) (AA) — ✓ PASS

**Evidence:** Color/variant matrix uses semantic tokens —
`lib/pulsar/components/input.ex`, `get_color_classes/2`,
`get_decorator_color_classes/2`. Browser measurement of 361 cells per
theme ([light](measurements/input-light.md),
...
`lib/pulsar/components/input.ex`, `get_container_size_classes/1`.
Padding and text classes also rem.
...
`lib/pulsar/components/input.ex`, `get_classes/3`. Input itself uses
`w-full` (`px-*` padding only) — `render_styled_input/2`. No fixed widths
or min-widths.
...
- Outline variant uses `border-2` with semantic border colors —
  `lib/pulsar/components/input.ex`, `get_classes/3`, `get_color_classes/2`
...
`lib/pulsar/components/input.ex`, `get_container_size_classes/1`.
Padding `px-*`/`py-*` is rem. Browser test injects the four WCAG 1.4.12
...
`lib/pulsar/components/input.ex`, `render_styled_input/2`. No custom
keydown handlers.
...
ease-standard`) — `lib/pulsar/components/input.ex`, `get_classes/3`,
`render_styled_input/2`. No essential motion.

### 2.3.1 Three Flashes or Below Threshold (A) — ✓ PASS

**Evidence:** No flashing; only smooth transitions —
`lib/pulsar/components/input.ex`, `get_classes/3`, `render_styled_input/2`.
...
focus in DOM order — `lib/pulsar/components/input.ex`, `start_decorator/1`,
`end_decorator/1`.
...
container — `lib/pulsar/components/input.ex`, `get_classes/3`. Native
input is inside; container focus-within fires when the input is focused.
...
handlers — `lib/pulsar/components/input.ex`, `render_styled_input/2`.

### 2.5.3 Label in Name (A) — ✓ PASS

**Evidence:** Input does not set its own `aria-label`; accessible name
flows from the associated `<label>` — `lib/pulsar/components/input.ex`,
`render_styled_input/2`.

### 2.5.8 Target Size (Minimum) (AA, new in 2.2) — ✓ PASS

**Evidence:** Size `xs` is `min-h-6` (24px) —
`lib/pulsar/components/input.ex`, `get_container_size_classes/1`. Other
sizes (`sm`=32px, `md`=40px, `lg`=48px, `xl`=56px) exceed. Browser
measurement of 361 fixture cells: 361/361 pass ≥24×24
...
`lib/pulsar/components/input.ex`, `input/1` (attr declaration),
`render_styled_input/2` (forwarded via `{@rest}`).

### 3.3.1 Error Identification (A) — ✓ PASS

**Evidence:** `aria-invalid="true"`/`"false"` written based on errors —
`lib/pulsar/components/input.ex`, `render_styled_input/2`. Error message
rendering happens at the `field` wrapper level.
...
- Role: implicit from native `<input type="…">` —
  `lib/pulsar/components/input.ex`, `render_styled_input/2`
- Name: from `name=` attr (Phoenix form integration) — `render_styled_input/2`
- Value: from `value=` attr (skipped for `type="file"` per HTML
  semantics) — `render_styled_input/2`
...
`lib/pulsar/components/input.ex`, `render_styled_input/2`. The
