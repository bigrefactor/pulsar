...
- Native `<button>` element preserves semantic role —
  `lib/pulsar/components/button.ex`, `render_button/1`
- Pseudo-button (`as: :a`, `as: :div`) explicitly applies
  `role="button"` — `lib/pulsar/components/button.ex`, `render_button/1`
...
`lib/pulsar/components/button.ex`, `render_button_content/1`.
...
- Loading state combines spinner + opacity + `cursor-wait` —
  `lib/pulsar/components/button.ex`, `base_button_classes/0`, `render_button_content/1`
- Disabled state combines opacity + `cursor-not-allowed` + native
  `disabled` attr — `lib/pulsar/components/button.ex`, `base_button_classes/0`, `render_button/1`
...
`lib/pulsar/components/button.ex`, `base_button_classes/0`, `render_button/1`.
...
`lib/pulsar/components/button.ex`, `color_classes/2`. Browser measurements via
...
sizes — `lib/pulsar/components/button.ex`, `size_classes/1`.
...
`lib/pulsar/components/button.ex`, `base_button_classes/0`. Outline variant uses
`border-2` in the active color — `lib/pulsar/components/button.ex`, `color_classes/2`.
...
`lib/pulsar/components/button.ex`, `size_classes/1`. Browser test injects WCAG
...
- Pseudo-button JS hook handles `keydown` Space/Enter and `keyup`
  Space to trigger `click()` — `lib/pulsar/components/button.ex`, `button_hook/1`
- Hook respects `aria-disabled` / `aria-busy` to short-circuit
  activation — `lib/pulsar/components/button.ex`, `button_hook/1`
...
Space/Enter — `lib/pulsar/components/button.ex`, `button_hook/1`.
...
scales. No flashing — `lib/pulsar/components/button.ex`, `base_button_classes/0`,
`variant_classes/1`, `render_button_content/1`.
...
`lib/pulsar/components/button.ex`, `button/1`. Buttons with navigation
...
- Browser measurement: `--color-ring` resolves to 5.02:1 (light) and
  ...
  `not-focusable-in-state` (`tabindex="-1"` per `tabindex/1`),
...
icon-only buttons" — `lib/pulsar/components/button.ex`, `button/1`. Default
...
**Evidence:** Size `xs` is `h-6` (24px) — `lib/pulsar/components/button.ex`, `size_classes/1`,
...
component template — `lib/pulsar/components/button.ex`, `button/1`.
...
- Disclosure linkage enforced: raises if `:expanded` set without
  `:controls` — `lib/pulsar/components/button.ex`, `ensure_disclosure_linkage!/1`
- Tests assert `aria-busy`, `aria-disabled`, `aria-pressed`, `role="button"` —
  Test `renders role="button" when rendered as a non-button element`,
  Test `renders aria-busy="true" while loading`,
  Test `renders aria-disabled="true" when disabled`,
  Test `renders aria-pressed reflecting the pressed value` —
  `test/pulsar/components/button_test.exs`
...
- `aria-busy="true"` rendered while loading —
  `lib/pulsar/components/button.ex`, `render_button/1`, `render_navigation_link/1`
- Test `renders aria-busy="true" while loading` — `test/pulsar/components/button_test.exs`
