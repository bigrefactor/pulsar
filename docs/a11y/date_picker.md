...
`name="hero-calendar"` — `lib/pulsar/components/date_picker.ex`, `date_picker/1`. The
button carries `aria-label="Open calendar"` which provides the accessible name
for the icon-only control — `lib/pulsar/components/date_picker.ex`, `date_picker/1`. The
...
- The range-separator dash is `aria-hidden="true"` so it is invisible to
  assistive technologies — `lib/pulsar/components/date_picker.ex`, `date_picker/1`
...
`lib/pulsar/components/date_picker.ex`, `date_picker/1`. The hidden ISO inputs live
...
display input(s) — `lib/pulsar/components/date_picker.ex`, `date_picker/1` — and
`border-danger` on the wrapper — `lib/pulsar/components/date_picker.ex`, `wrapper_classes/2`.
...
on the display input(s) — `lib/pulsar/components/date_picker.ex`, `date_picker/1`,
`wrapper_classes/2`.
Disabled state uses the native `disabled` attribute on both the display inputs
and the calendar trigger button — `lib/pulsar/components/date_picker.ex`, `date_picker/1`. Color reinforces these states but is never the sole signal.
...
`lib/pulsar/components/date_picker.ex`, `input_classes/1`. `text-foreground` maps to
...
`border-border bg-background` — `lib/pulsar/components/date_picker.ex`, `wrapper_classes/2`.
...
`lib/pulsar/components/date_picker.ex`, `input_classes/1`. Heights (`h-7` through `h-11`)
...
`lib/pulsar/components/date_picker.ex`, `input_classes/1`.

### 1.4.10 Reflow (AA) — ✓ PASS

**Evidence:** The outer container is `inline-flex items-center gap-2` with no
enforced minimum width — `lib/pulsar/components/date_picker.ex`, `date_picker/1`. The inner
wrapper is `inline-flex items-center gap-1.5 rounded-field px-2` —
`lib/pulsar/components/date_picker.ex`, `wrapper_classes/2`. Display inputs are `w-28`
...
- Outline variant wrapper border uses `border-border` (`gray-200` light /
  `gray-800` dark) — `lib/pulsar/components/date_picker.ex`, `wrapper_classes/2`.
...
- The focus ring on the wrapper uses `focus-within:ring-2 focus-within:ring-ring` —
  `lib/pulsar/components/date_picker.ex`, `wrapper_classes/2`. `--color-ring` measures 5.02:1
  (light) / 6.72:1 (dark) against `bg-background`.
- The calendar trigger button uses `focus-visible:ring-2 focus-visible:ring-ring` —
  `lib/pulsar/components/date_picker.ex`, `date_picker/1`. Same token; same measured values.

### 1.4.12 Text Spacing (AA) — ✓ PASS

**Evidence:** No `!important` overrides on spacing. Input sizes use
Tailwind utility classes — `lib/pulsar/components/date_picker.ex`, `input_classes/1`. Text is not locked inside a pixel-height container that would clip
...
- **Type-in path:** Display inputs are native `<input type="text">` — fully
  ...
  `lib/pulsar/components/date_picker.ex`, `date_picker/1`. Invalid input sets
  `aria-invalid="true"` so the user sees the parse failure.
- **Calendar trigger button:** Native `<button type="button">` — fully keyboard
  operable — `lib/pulsar/components/date_picker.ex`, `date_picker/1`.
...
`lib/pulsar/components/date_picker.ex`, `date_picker/1`. The popover focus trap behavior is
...
document tab order — `lib/pulsar/components/date_picker.ex`, `date_picker/1`.
...
- Single mode: `aria-label={@display_label || (!@labelled_externally && "Date")}` —
  `lib/pulsar/components/date_picker.ex`, `date_picker/1`. Standalone: `"Date"` (or
...
- Range start: `aria-label={!@labelled_externally && "Start date"}` —
  `lib/pulsar/components/date_picker.ex`, `date_picker/1`. Standalone: `"Start date"`.
...
- The input wrapper gains `focus-within:ring-2 focus-within:ring-ring` when any
  child receives focus — `lib/pulsar/components/date_picker.ex`, `wrapper_classes/2`. This
  ...
  wrapper ring — `lib/pulsar/components/date_picker.ex`, `input_classes/1`.
...
the `click` event (fires on mouseup) — `lib/pulsar/components/date_picker.ex`,
`date_picker/1`.
...
`lib/pulsar/components/date_picker.ex`, `date_picker/1`. No `mousedown` or
...
complete accessible name — `lib/pulsar/components/date_picker.ex`, `date_picker/1`.
...
- Range start: `aria-label={!@labelled_externally && "Start date"}` → evaluates
  to `false` — `lib/pulsar/components/date_picker.ex`, `date_picker/1`
...
`lib/pulsar/components/date_picker.ex`, `date_picker/1`. No external `<label>` targets this
...
`Icon` of `size="sm"` (20×20 CSS px) — `lib/pulsar/components/date_picker.ex`,
`date_picker/1`.
...
blur, not on each keystroke) — `lib/pulsar/components/date_picker.ex`, `date_picker/1`.
The `on_change` `%JS{}` callback is executed only after a successfully parsed
date is written — `lib/pulsar/components/date_picker.ex`, `date_picker/1`.
...
2. Client-side by the hook, when a typed value cannot be parsed to a valid
   ISO date — `lib/pulsar/components/date_picker.ex`, `date_picker/1`

`aria-invalid` returns to `"false"` when the value is cleared or a valid date
is parsed — `lib/pulsar/components/date_picker.ex`, `date_picker/1`. When used via
...
`aria-label` values at `lib/pulsar/components/date_picker.ex`, `date_picker/1`.
Via `Field`: the single/start input's default `aria-label` is suppressed via
`labelled_externally` — `lib/pulsar/components/date_picker.ex`, `date_picker/1`
(`attr :labelled_externally`) — and the
...
- Calendar trigger button: `<button type="button">` with `aria-label="Open
  calendar"` — `lib/pulsar/components/date_picker.ex`, `date_picker/1`. `Popover`
...
`lib/pulsar/components/date_picker.ex`, `date_picker/1`. The
...
- **2.4.13 Focus Appearance (AAA, new in 2.2)** — `focus-within:ring-2` on the
  ...
  `lib/pulsar/components/date_picker.ex`, `date_picker/1`, `wrapper_classes/2`.
