...
`name="hero-calendar"` — `lib/pulsar/components/date_picker.ex:166`. The
button carries `aria-label="Open calendar"` which provides the accessible name
for the icon-only control — `lib/pulsar/components/date_picker.ex:162`. The
...
- Single-mode display input: `type="text"`, `aria-label={@display_label ||
  ...
  `lib/pulsar/components/date_picker.ex:117`
...
is not caller-overridable — `lib/pulsar/components/date_picker.ex:145`. This is
...
`lib/pulsar/components/date_picker.ex:97–187`. The hidden ISO inputs live
...
display input(s) — `lib/pulsar/components/date_picker.ex:118, 132, 146` — and
`border-danger` on the wrapper — `lib/pulsar/components/date_picker.ex:410`.
...
on the display input(s) — `lib/pulsar/components/date_picker.ex:118, 132, 146, 410`.
Disabled state uses the native `disabled` attribute on both the display inputs
and the calendar trigger button — `lib/pulsar/components/date_picker.ex:116, 130,
144, 163`. Color reinforces these states but is never the sole signal.
...
`lib/pulsar/components/date_picker.ex:400`. `text-foreground` maps to
...
`lib/pulsar/components/date_picker.ex:400`.

### 1.4.10 Reflow (AA) — ✓ PASS

**Evidence:** The outer container is `inline-flex items-center gap-2` with no
enforced minimum width — `lib/pulsar/components/date_picker.ex:105`. The inner
wrapper is `inline-flex items-center gap-1.5 rounded-field px-2` —
`lib/pulsar/components/date_picker.ex:408`. Display inputs are `w-28`
...
- The focus ring on the wrapper uses `focus-within:ring-2 focus-within:ring-ring` —
  `lib/pulsar/components/date_picker.ex:408`. `--color-ring` measures 5.02:1
  (light) / 6.72:1 (dark) against `bg-background`.
- The calendar trigger button uses `focus-visible:ring-2 focus-visible:ring-ring` —
  `lib/pulsar/components/date_picker.ex:164`. Same token; same measured values.
...
400`. Text is not locked inside a pixel-height container that would clip
...
`lib/pulsar/components/date_picker.ex:238`. The popover focus trap behavior is
...
document tab order — `lib/pulsar/components/date_picker.ex:97–187`.
...
- Single mode: `aria-label={@display_label || (!@labelled_externally && "Date")}` —
  `lib/pulsar/components/date_picker.ex:117`. Standalone: `"Date"` (or
...
- Range start: `aria-label={!@labelled_externally && "Start date"}` —
  `lib/pulsar/components/date_picker.ex:131`. Standalone: `"Start date"`.
...
- The input wrapper gains `focus-within:ring-2 focus-within:ring-ring` when any
  child receives focus — `lib/pulsar/components/date_picker.ex:408`. This
  ...
  wrapper ring — `lib/pulsar/components/date_picker.ex:400`.
...
the `click` event (fires on mouseup) — `lib/pulsar/components/date_picker.ex:160`.
...
`lib/pulsar/components/date_picker.ex:238, 246`. No `mousedown` or
...
complete accessible name — `lib/pulsar/components/date_picker.ex:162`.
...
- Range start: `aria-label={!@labelled_externally && "Start date"}` → evaluates
  to `false` — `lib/pulsar/components/date_picker.ex:131`
...
`lib/pulsar/components/date_picker.ex:145`. No external `<label>` targets this
...
`Icon` of `size="sm"` (20×20 CSS px) — `lib/pulsar/components/date_picker.ex:164–166`.
...
`lib/pulsar/components/date_picker.ex:191–246`.

### 3.2.2 On Input (A) — ✓ PASS

**Evidence:** Parsing and ISO-write happen on the `change` event (fired on
blur, not on each keystroke) — `lib/pulsar/components/date_picker.ex:238`.
The `on_change` `%JS{}` callback is executed only after a successfully parsed
date is written — `lib/pulsar/components/date_picker.ex:227–230, 354–357`.
...
2. Client-side by the hook, when a typed value cannot be parsed to a valid
   ISO date — `lib/pulsar/components/date_picker.ex:235`

`aria-invalid` returns to `"false"` when the value is cleared or a valid date
is parsed — `lib/pulsar/components/date_picker.ex:223, 233`. When used via
...
`aria-label` values at `lib/pulsar/components/date_picker.ex:117, 131, 145`.
...
- Display input(s): native `<input type="text">` with conditional `aria-label`
  ...
  `lib/pulsar/components/date_picker.ex:109–150`
- Calendar trigger button: `<button type="button">` with `aria-label="Open
  calendar"` — `lib/pulsar/components/date_picker.ex:160–167`. `Popover`
...
- Hidden ISO inputs: `type="hidden"` — not exposed in the accessibility tree;
  ...
  `lib/pulsar/components/date_picker.ex:185–187`
...
`lib/pulsar/components/date_picker.ex:118, 132, 146, 223, 233, 235`. The
...
- **2.4.13 Focus Appearance (AAA, new in 2.2)** — `focus-within:ring-2` on the
  ...
  `lib/pulsar/components/date_picker.ex:164, 408`.
