# Flash — a11y measurements (dark)

Auto-generated by `mix pulsar.a11y.measure`. Do not edit by hand —
re-run the task to refresh. The canonical audit page is
[`flash.md`](../flash.md).

- **Theme:** `dark`
- **Viewport:** 1280×720 CSS px
- **Cells:** 55

## Per-cell measurements

Columns:

- **Size** — `getBoundingClientRect()` width × height. ≥24 column flags
  pass under WCAG 2.5.8 Target Size minimum.
- **Text** — text-color vs effective background contrast ratio
  (alpha-resolved). Threshold 4.5:1 (3:1 for large text). `—` means no
  text or no resolvable color. For mask-painted icon glyphs the value is
  the painted glyph color vs the *ancestor* background at the 3:1 WCAG
  1.4.11 non-text threshold, marked `(glyph)`; decorative
  (`aria-hidden`) icons are exempt and marked `(glyph, decorative)`.
- **Border** — border-color vs adjacent background contrast. `—` if no
  visible border. Threshold 3:1 per WCAG 1.4.11 Non-text Contrast.
- **Focus** — focus-visible ring/outline vs adjacent background.
  Threshold 3:1 per WCAG 2.4.7 / 1.4.11.

| Cell ID | Tag | Width | Height | ≥24×24 | Text | Border | Focus |
|---------|-----|-------|--------|--------|------|--------|-------|
| `dismissible` | `div` | 245.97 | 48 | ✓ | 11.32:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-danger-lg` | `div` | 473 | 56 | ✓ | 7.74:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-danger-md` | `div` | 473 | 48 | ✓ | 7.74:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-danger-sm` | `div` | 473 | 48 | ✓ | 7.74:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-neutral-lg` | `div` | 473 | 56 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-neutral-md` | `div` | 473 | 48 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-neutral-sm` | `div` | 473 | 48 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-primary-lg` | `div` | 473 | 56 | ✓ | 5.78:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-primary-md` | `div` | 473 | 56 | ✓ | 5.78:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-primary-sm` | `div` | 473 | 56 | ✓ | 5.78:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-secondary-lg` | `div` | 473 | 56 | ✓ | 5.4:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-secondary-md` | `div` | 473 | 48 | ✓ | 5.4:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-secondary-sm` | `div` | 473 | 48 | ✓ | 5.4:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-success-lg` | `div` | 473 | 56 | ✓ | 8.24:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-success-md` | `div` | 473 | 56 | ✓ | 8.24:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-success-sm` | `div` | 473 | 56 | ✓ | 8.24:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-warning-lg` | `div` | 473 | 56 | ✓ | 8.54:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-warning-md` | `div` | 473 | 56 | ✓ | 8.54:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `ghost-warning-sm` | `div` | 473 | 56 | ✓ | 8.54:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `outline-danger-lg` | `div` | 473 | 58 | ✓ | 10.49:1 ✓ | 9.25:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-danger-md` | `div` | 473 | 50 | ✓ | 10.49:1 ✓ | 9.25:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-danger-sm` | `div` | 473 | 50 | ✓ | 10.49:1 ✓ | 9.25:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-neutral-lg` | `div` | 473 | 58 | ✓ | 19.27:1 ✓ | 3.67:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-neutral-md` | `div` | 473 | 50 | ✓ | 19.27:1 ✓ | 3.67:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-neutral-sm` | `div` | 473 | 50 | ✓ | 19.27:1 ✓ | 3.67:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-primary-lg` | `div` | 473 | 58 | ✓ | 7.63:1 ✓ | 6.72:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-primary-md` | `div` | 473 | 58 | ✓ | 7.63:1 ✓ | 6.72:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-primary-sm` | `div` | 473 | 58 | ✓ | 7.63:1 ✓ | 6.72:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-secondary-lg` | `div` | 473 | 58 | ✓ | 7.06:1 ✓ | 6.22:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-secondary-md` | `div` | 473 | 50 | ✓ | 7.06:1 ✓ | 6.22:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-secondary-sm` | `div` | 473 | 50 | ✓ | 7.06:1 ✓ | 6.22:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-success-lg` | `div` | 473 | 58 | ✓ | 11.32:1 ✓ | 9.98:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-success-md` | `div` | 473 | 58 | ✓ | 11.32:1 ✓ | 9.98:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-success-sm` | `div` | 473 | 58 | ✓ | 11.32:1 ✓ | 9.98:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-warning-lg` | `div` | 473 | 58 | ✓ | 11.69:1 ✓ | 10.3:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-warning-md` | `div` | 473 | 58 | ✓ | 11.69:1 ✓ | 10.3:1 ✓ (top) | — (not-focusable-in-state) |
| `outline-warning-sm` | `div` | 473 | 58 | ✓ | 11.69:1 ✓ | 10.3:1 ✓ (top) | — (not-focusable-in-state) |
| `solid-danger-lg` | `div` | 473 | 56 | ✓ | 10.49:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-danger-md` | `div` | 473 | 48 | ✓ | 10.49:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-danger-sm` | `div` | 473 | 48 | ✓ | 10.49:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-neutral-lg` | `div` | 473 | 56 | ✓ | 4.84:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-neutral-md` | `div` | 473 | 48 | ✓ | 4.84:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-neutral-sm` | `div` | 473 | 48 | ✓ | 4.84:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-primary-lg` | `div` | 473 | 56 | ✓ | 7.63:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-primary-md` | `div` | 473 | 56 | ✓ | 7.63:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-primary-sm` | `div` | 473 | 56 | ✓ | 7.63:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-secondary-lg` | `div` | 473 | 56 | ✓ | 7.06:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-secondary-md` | `div` | 473 | 48 | ✓ | 7.06:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-secondary-sm` | `div` | 473 | 48 | ✓ | 7.06:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-success-lg` | `div` | 473 | 56 | ✓ | 11.32:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-success-md` | `div` | 473 | 56 | ✓ | 11.32:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-success-sm` | `div` | 473 | 56 | ✓ | 11.32:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-warning-lg` | `div` | 473 | 56 | ✓ | 11.69:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-warning-md` | `div` | 473 | 56 | ✓ | 11.69:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `solid-warning-sm` | `div` | 473 | 56 | ✓ | 11.69:1 ✓ | — (no-border) | — (not-focusable-in-state) |

## Text-spacing override (WCAG 1.4.12)

Applied: `line-height: 1.5`, `letter-spacing: 0.12em`,
`word-spacing: 0.16em`, `p { margin-bottom: 2em }`.

**Cells overflowing:** none.

## Reflow at 320 CSS px (WCAG 1.4.10)

Constraint applied via `html, body { width: 320px }`.
Media queries do not trigger under CSS-only constraint — interpret
overflows as worst-case, not as media-query-aware behavior.

- **Page horizontally overflows:** yes

**Cells overflowing (54):**

- `solid-neutral-sm` (x) — content 55×96 vs box 16×96
- `solid-neutral-md` (x) — content 64×96 vs box 24×96
- `solid-neutral-lg` (x) — content 68×104 vs box 32×104
- `solid-primary-sm` (x) — content 59×104 vs box 16×104
- `solid-primary-md` (x) — content 70×104 vs box 24×104
- `solid-primary-lg` (x) — content 74×104 vs box 32×104
- `solid-secondary-sm` (x) — content 78×96 vs box 16×96
- `solid-secondary-md` (x) — content 90×96 vs box 24×96
- `solid-secondary-lg` (x) — content 94×104 vs box 32×104
- `solid-success-sm` (x) — content 62×104 vs box 16×104
- `solid-success-md` (x) — content 73×104 vs box 24×104
- `solid-success-lg` (x) — content 77×104 vs box 32×104
- `solid-danger-sm` (x) — content 55×96 vs box 16×96
- `solid-danger-md` (x) — content 65×96 vs box 24×96
- `solid-danger-lg` (x) — content 69×104 vs box 32×104
- `solid-warning-sm` (x) — content 61×104 vs box 16×104
- `solid-warning-md` (x) — content 72×104 vs box 24×104
- `solid-warning-lg` (x) — content 76×104 vs box 32×104
- `outline-neutral-sm` (x) — content 55×96 vs box 16×96
- `outline-neutral-md` (x) — content 64×96 vs box 24×96
- `outline-neutral-lg` (x) — content 68×104 vs box 32×104
- `outline-primary-sm` (x) — content 59×104 vs box 16×104
- `outline-primary-md` (x) — content 70×104 vs box 24×104
- `outline-primary-lg` (x) — content 74×104 vs box 32×104
- `outline-secondary-sm` (x) — content 78×96 vs box 16×96
- `outline-secondary-md` (x) — content 90×96 vs box 24×96
- `outline-secondary-lg` (x) — content 94×104 vs box 32×104
- `outline-success-sm` (x) — content 62×104 vs box 16×104
- `outline-success-md` (x) — content 73×104 vs box 24×104
- `outline-success-lg` (x) — content 77×104 vs box 32×104
- `outline-danger-sm` (x) — content 55×96 vs box 16×96
- `outline-danger-md` (x) — content 65×96 vs box 24×96
- `outline-danger-lg` (x) — content 69×104 vs box 32×104
- `outline-warning-sm` (x) — content 61×104 vs box 16×104
- `outline-warning-md` (x) — content 72×104 vs box 24×104
- `outline-warning-lg` (x) — content 76×104 vs box 32×104
- `ghost-neutral-sm` (x) — content 55×96 vs box 16×96
- `ghost-neutral-md` (x) — content 64×96 vs box 24×96
- `ghost-neutral-lg` (x) — content 68×104 vs box 32×104
- `ghost-primary-sm` (x) — content 59×104 vs box 16×104
- `ghost-primary-md` (x) — content 70×104 vs box 24×104
- `ghost-primary-lg` (x) — content 74×104 vs box 32×104
- `ghost-secondary-sm` (x) — content 78×96 vs box 16×96
- `ghost-secondary-md` (x) — content 90×96 vs box 24×96
- `ghost-secondary-lg` (x) — content 94×104 vs box 32×104
- `ghost-success-sm` (x) — content 62×104 vs box 16×104
- `ghost-success-md` (x) — content 73×104 vs box 24×104
- `ghost-success-lg` (x) — content 77×104 vs box 32×104
- `ghost-danger-sm` (x) — content 55×96 vs box 16×96
- `ghost-danger-md` (x) — content 65×96 vs box 24×96
- `ghost-danger-lg` (x) — content 69×104 vs box 32×104
- `ghost-warning-sm` (x) — content 61×104 vs box 16×104
- `ghost-warning-md` (x) — content 72×104 vs box 24×104
- `ghost-warning-lg` (x) — content 76×104 vs box 32×104
