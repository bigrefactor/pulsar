# Header — a11y measurements (dark)

Auto-generated by `mix pulsar.a11y.measure`. Do not edit by hand —
re-run the task to refresh. The canonical audit page is
[`header.md`](../header.md).

- **Theme:** `dark`
- **Viewport:** 1280×720 CSS px
- **Cells:** 20

## Per-cell measurements

Columns:

- **Size** — `getBoundingClientRect()` width × height. ≥24 column flags
  pass under WCAG 2.5.8 Target Size minimum.
- **Text** — text-color vs effective background contrast ratio
  (alpha-resolved). Threshold 4.5:1 (3:1 for large text). `—` means no
  text or no resolvable color. For mask-painted icon glyphs the value is
  the painted glyph color vs the *ancestor* background at the 3:1 WCAG
  1.4.11 non-text threshold, marked `(glyph)`; decorative
  (`aria-hidden`) icons are exempt and marked `(glyph, decorative)`.
- **Border** — border-color vs adjacent background contrast. `—` if no
  visible border. Threshold 3:1 per WCAG 1.4.11 Non-text Contrast.
- **Focus** — focus-visible ring/outline vs adjacent background.
  Threshold 3:1 per WCAG 2.4.7 / 1.4.11.

| Cell ID | Tag | Width | Height | ≥24×24 | Text | Border | Focus |
|---------|-----|-------|--------|--------|------|--------|-------|
| `actions` | `header` | 285.03 | 56 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `as-h1` | `header` | 958 | 32 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `as-h2` | `header` | 958 | 32 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `as-h3` | `header` | 958 | 32 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `as-h4` | `header` | 958 | 32 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `as-h5` | `header` | 958 | 32 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `as-h6` | `header` | 958 | 32 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `breadcrumbs` | `header` | 194.53 | 68 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `outline-danger` | `header` | 958 | 73 | ✓ | 9.25:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `outline-info` | `header` | 958 | 73 | ✓ | 9.81:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `outline-neutral` | `header` | 958 | 73 | ✓ | 16.98:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `outline-primary` | `header` | 958 | 73 | ✓ | 6.72:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `outline-secondary` | `header` | 958 | 73 | ✓ | 6.22:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `outline-success` | `header` | 958 | 73 | ✓ | 9.98:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `outline-warning` | `header` | 958 | 73 | ✓ | 10.3:1 ✓ | 6.82:1 ✓ (bottom) | — (not-focusable-in-state) |
| `size-lg` | `header` | 958 | 64 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `size-md` | `header` | 958 | 56 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `size-sm` | `header` | 958 | 52 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `size-xl` | `header` | 958 | 72 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |
| `size-xs` | `header` | 958 | 48 | ✓ | 16.98:1 ✓ | — (no-border) | — (not-focusable-in-state) |

## Text-spacing override (WCAG 1.4.12)

Applied: `line-height: 1.5`, `letter-spacing: 0.12em`,
`word-spacing: 0.16em`, `p { margin-bottom: 2em }`.

**Cells overflowing:** none.

## Reflow at 320 CSS px (WCAG 1.4.10)

Constraint applied via `html, body { width: 320px }`.
Media queries do not trigger under CSS-only constraint — interpret
overflows as worst-case, not as media-query-aware behavior.

- **Page horizontally overflows:** yes

**Cells overflowing (18):**

- `size-xs` (x) — content 62×120 vs box 0×120
- `size-sm` (x) — content 67×128 vs box 0×128
- `size-md` (x) — content 80×140 vs box 0×140
- `size-lg` (x) — content 100×160 vs box 0×160
- `size-xl` (x) — content 120×180 vs box 0×180
- `as-h1` (x) — content 25×32 vs box 0×32
- `as-h2` (x) — content 28×32 vs box 0×32
- `as-h3` (x) — content 29×32 vs box 0×32
- `as-h4` (x) — content 29×32 vs box 0×32
- `as-h5` (x) — content 29×32 vs box 0×32
- `as-h6` (x) — content 30×32 vs box 0×32
- `outline-neutral` (x) — content 79×156 vs box 0×156
- `outline-primary` (x) — content 84×156 vs box 0×156
- `outline-secondary` (x) — content 115×156 vs box 0×156
- `outline-success` (x) — content 90×156 vs box 0×156
- `outline-danger` (x) — content 79×156 vs box 0×156
- `outline-warning` (x) — content 88×156 vs box 0×156
- `outline-info` (x) — content 79×156 vs box 0×156
