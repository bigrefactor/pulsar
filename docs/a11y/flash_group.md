...
etc.) — `lib/pulsar/components/flash_group.ex`, `get_flash_icon/1`,
`flash_group/1`. The `Pulsar.Components.Icon.icon` component handles its
own `aria-hidden`/`aria-label`.
...
- Each flash gets `role={get_flash_role(type)}` — `alert` for error and
  warning, `status` for the rest — `lib/pulsar/components/flash_group.ex`,
  `get_flash_role/1`, `flash_group/1`
...
semantic icon paired with text — `lib/pulsar/components/flash_group.ex`,
`get_flash_icon/1`. Type-to-role mapping (`alert` vs `status`) provides a
non-visual criticality signal — `get_flash_role/1`.
...
(color, icon, role, text content) — `lib/pulsar/components/flash_group.ex`,
`get_flash_color/1`, `get_flash_icon/1`, `flash_group/1`.

### 1.4.10 Reflow (AA) — ✓ PASS

**Evidence:** Container uses `max-w-sm w-full` on all positions —
`lib/pulsar/components/flash_group.ex`, `get_position_config/1`. At 320
CSS px viewport, `max-w-sm` (24rem = 384px) is capped by `w-full` to the
viewport width.

**Notes:** `top-center` and `bottom-center` use
`left-1/2 -translate-x-1/2` which centers properly at all widths —
`get_position_config/1`. Left and right positions use
`left-4`/`right-4` (1rem inset) and reflow gracefully.
...
its own — `lib/pulsar/components/flash_group.ex`, `flash_group/1`.
Browser measurement: 6 cells, no borders, no focus rings to evaluate
...
`lib/pulsar/components/flash_group.ex`, `get_position_config/1`. No
`!important` on spacing.

### 1.4.13 Content on Hover or Focus (AA) — ✓ PASS

**Evidence:** Container is `pointer-events-none` so it doesn't intercept
hover — `lib/pulsar/components/flash_group.ex`, `flash_group/1`.
Hover/focus behavior on individual flashes is covered in `flash.md`
1.4.13.

### 2.1.1 Keyboard (A) — ✓ PASS

**Evidence:** Container has no interactive elements of its own —
`lib/pulsar/components/flash_group.ex`, `flash_group/1`. Each child
flash's close button is keyboard-operable (covered in `flash.md` 2.1.1).

### 2.1.2 No Keyboard Trap (A) — ✓ PASS

**Evidence:** Container has no focus handlers; child flashes don't
trap Tab — `lib/pulsar/components/flash_group.ex`, `flash_group/1`,
...
`lib/pulsar/components/flash_group.ex`, `flash_group/1` (attr
declarations). Per-child behavior covered in `flash.md` 2.2.1.
...
ms then settles; no continuous motion — `lib/pulsar/components/flash_group.ex`,
`stagger_ms/2`. Hide via dismiss covered in `flash.md` 2.2.2.

### 2.3.1 Three Flashes or Below Threshold (A) — ✓ PASS

**Evidence:** Only animations are CSS `transition` on opacity/translate
in `phx-mounted` JS hooks — `lib/pulsar/components/flash_group.ex`,
`flash_group/1`. No flashing/blinking.

### 2.4.3 Focus Order (A) — ✓ PASS

**Evidence:**
- Messages render in deterministic priority order in the DOM —
  `lib/pulsar/components/flash_group.ex`, `flash_priority/1`
- No positive `tabindex` values used — only the close buttons inside
  child flashes are focusable, in DOM order — `flash_group/1`
...
`lib/pulsar/components/flash_group.ex`, `flash_group/1`. Focus
visibility on child close buttons covered in `flash.md` 2.4.7.
...
`z-50` default — `lib/pulsar/components/flash_group.ex`, `flash_group/1`
(attr declaration), `get_position_config/1`, `get_z_index_class/1`.
`pointer-events-none` on the container — `flash_group/1` — means clicks
pass through, but focus visibility under the flash stack is still a
...
covered in `flash.md` 2.5.2 — `lib/pulsar/components/flash_group.ex`,
`flash_group/1`.

### 2.5.8 Target Size (Minimum) (AA, new in 2.2) — ✓ PASS

**Evidence:** Container has no interactive targets —
`lib/pulsar/components/flash_group.ex`, `flash_group/1`. Child
close-button target sizing is covered in `flash.md` 2.5.8.

### 3.2.1 On Focus (A) — ✓ PASS

**Evidence:** No focus handlers on the container —
`lib/pulsar/components/flash_group.ex`, `flash_group/1`.

### 4.1.2 Name, Role, Value (A) — ✓ PASS

**Evidence:** Container is a plain `<div>` with no implicit interactive
role — `lib/pulsar/components/flash_group.ex`, `flash_group/1`. Each
child flash carries `role`, `aria-live`, `aria-atomic`, and the dismiss
button carries `aria-label` and `aria-controls` (see `flash.md` 4.1.2).
...
component — `lib/pulsar/components/flash_group.ex`, `get_flash_role/1`,
`flash_group/1`; tests assert alert vs status by type —
`test/pulsar/components/flash_group_test.exs`.
