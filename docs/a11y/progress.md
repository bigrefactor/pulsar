...
(`lib/pulsar/components/progress.ex`, `progress/1`),
not as images. The radial SVG is presentational inside the named progressbar; the
visible percentage text is `aria-hidden` (`lib/pulsar/components/progress.ex`,
`progress/1` for both the linear bar and the radial ring)
...
(`lib/pulsar/components/progress.ex`, `progress/1`) expose the progress relationship to
assistive tech. Indeterminate omits `aria-valuenow`
(`progress/1`, resolved to `nil` by `progress_values/2` in
`lib/pulsar/components/progress.ex`).
...
name (`lib/pulsar/components/progress.ex`, `progress/1`); the semantic color palette
(`lib/pulsar/components/progress.ex`, `assign_classes/1` — determinate bar fill,
indeterminate bar fill, and ring colors) is decorative on top of
that role/value.
...
surfaces), and the radial center value uses `text-foreground` — both in
`lib/pulsar/components/progress.ex`, `progress/1`. Verified by
...
track (`lib/pulsar/components/progress.ex`, `progress/1`); the radial ring uses
full-strength semantic stroke color against an `opacity-20` track ring
(`progress/1`), giving a discernible boundary between
filled and unfilled regions.

### 2.2.2 Pause, Stop, Hide (A) — ✓ PASS

**Evidence:** The indeterminate animation is a decorative `animate-pulse` opacity
loop (`lib/pulsar/components/progress.ex`, `progress/1`) on an element whose meaning is
...
**Evidence:** Role `progressbar` and
value from `aria-valuenow` (or absent for indeterminate) — both in
`lib/pulsar/components/progress.ex`, `progress/1`. The accessible name is a **caller
responsibility**: it derives from `label` (rendered and mirrored to `aria-label`)
or an explicit `aria-label`, also in `progress/1`. With
neither supplied the progressbar has no accessible name — the component documents
this contract in its `@moduledoc` (`lib/pulsar/components/progress.ex`), and
every shipped fixture and Storybook example provides a name, so the axe gate runs
only named instances. Callers embedding `<.progress>` must pass `label` or
`aria-label`.
