...
marked `aria-hidden="true"` — `lib/pulsar/components/tooltip.ex`, `tooltip/1`. The hint
...
`bg-{color} text-{color}-foreground` — `lib/pulsar/components/tooltip.ex`, `surface_classes/1`.
...
`lib/pulsar/components/tooltip.ex`, `panel_base_classes/0`.
...
`lib/pulsar/components/tooltip.ex`, `panel_base_classes/0`,
`lib/pulsar/components/popover.ex`, `popover/1` (`position`).
...
contrast of its own — `lib/pulsar/components/tooltip.ex`, `arrow_classes/0`. The tooltip surface
...
- **Hoverable:** leaving the trigger starts a short close timer rather than hiding
  ...
  onto the hint — `lib/pulsar/components/popover.ex`, `popover/1` (`_closeSoon`,
  panel `mouseenter` → `_cancelClose`).
- **Persistent:** it stays open while hovered/focused with no timeout; it closes
  only on leave, blur, or Escape — `lib/pulsar/components/popover.ex`, `popover/1`.
...
it immediately, and Escape dismisses it — `lib/pulsar/components/popover.ex`,
`popover/1` (`_openNow` on `focusin`; Escape). The keyboard fixture exercises
...
`lib/pulsar/components/tooltip.ex`, `tooltip/1`.
...
`tabindex` is used — `lib/pulsar/components/popover.ex`, `popover/1`.
...
`lib/pulsar/components/popover.ex`, `base_classes/0`.
...
side when the requested side lacks room — `lib/pulsar/components/popover.ex`,
`popover/1` (`position`). The trigger itself is never covered (the panel is offset from it).
...
`aria-describedby` pointing at it — `lib/pulsar/components/tooltip.ex`, `tooltip/1`,
`lib/pulsar/components/popover.ex`, `popover/1`. Unit tests assert the role and the
