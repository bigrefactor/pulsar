...
- **`checkbox`'s `card` layout no longer reacts to hover when disabled**: the card set `has-[:disabled]:cursor-not-allowed has-[:disabled]:opacity-disabled` but not `pointer-events-none`, so a disabled card still ran its `hover:bg-*` tint and (for `solid`/`outline`) its hover shadow — advertising a click that does nothing. Adding `has-[:disabled]:pointer-events-none` matches what `button`, `switch`, `input`, `select`, and `textarea` already do.
