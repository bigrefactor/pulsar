...
- **Form inputs derive their id from `name` before generating one**: `input`, `checkbox`, `switch`, `radio_group`, `select`, and `textarea` now resolve caller `id`, then `field.id`, then `name`, then a generated value. An unbound input with a `name` had a stable source going unused.
