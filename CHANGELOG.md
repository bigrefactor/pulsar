...
- **`alert`, `menu_section`, and `dropdown_menu_group` generate an id only when one is used**: `alert` when `dismissible` (for the dismiss button's `aria-controls`), the two section components when a `label` is present (for `aria-labelledby`).
  ...
  The other two are known gaps, not decisions, and closing either means a contract change. `input_otp` with neither a `field`, a `name`, nor an `id` derives its hook root's id (`"<id>-otp"`) from a generated value, and that hook holds the typed one-time code and the active-slot index — so a disconnected-to-connected swap remounts it and wipes a partially typed code. That makes it the more consequential of the two, and unlike `radio_group` the case is not degenerate: `<.input_otp on_complete={JS.push("verify")} />` is a legitimate call, because the hook pushes the value and nothing needs to submit. `radio_group` with neither a `field` nor a `name` also lands a generated id on a hook root; it was left alone because that case *is* degenerate (a group with no field and no name submits nowhere). Pass an explicit `id` to pin any of these six; for `radio_group` and `input_otp`, a `name` works too.
