...
- **`alert`, `menu_section`, and `dropdown_menu_group` generate an id only when one is used**: `alert` when `dismissible` (for the dismiss button's `aria-controls`), the two section components when a `label` is present (for `aria-labelledby`).
  ...
  Five branches still generate a per-render id, not fixed here. The three above do when the referencing markup is present, but nothing outside that render points at those ids, so the churn costs an attribute diff and no client state. `button` with `as={:a}` or `as={:div}` also does, and that one is on a `phx-hook` root — but the branches accept `href`/`navigate`, so requiring an id would break every `<.button href={...}>` call site, and the hook only attaches listeners, so remounting it loses nothing. The fifth is `radio_group` rendered with neither a `field` nor a `name`: its id also lands on a hook root, and unlike the other four this one is a known gap rather than a settled trade-off — it was left alone only because the case is degenerate (a group with no field and no name submits nowhere) and closing it means a contract change. Pass an explicit `id` to pin any of them; for `radio_group`, a `name` works too.
