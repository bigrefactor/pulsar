...
- **`alert`, `menu_section`, and `dropdown_menu_group` generate an id only when one is used**: `alert` when `dismissible` (for the dismiss button's `aria-controls`), the two section components when a `label` is present (for `aria-labelledby`).
  ...
  Four branches still generate a per-render id, accepted rather than fixed. The three above do when the referencing markup is present, but nothing outside that render points at those ids, so the churn costs an attribute diff and no client state. `button` with `as={:a}` or `as={:div}` also does, and that one is on a `phx-hook` root — but the branches accept `href`/`navigate`, so requiring an id would break every `<.button href={...}>` call site, and the hook only attaches listeners, so remounting it loses nothing. Pass an explicit `id` to pin any of them.
- **Form inputs derive their id from `name` before generating one**: `input`, `checkbox`, `switch`, `radio_group`, `select`, and `textarea` now resolve caller `id`, then `field.id`, then `name`, then — on the bound path only — a generated value. An unbound input with a `name` had a stable source going unused. `select` and `textarea` never reach a generated id when unbound: both require a `name` in that case, so the ladder ends at caller `id` then `name`.
...
- **`flash_group` derives child flash ids from a stable group id**: children were keyed on `System.unique_integer/1`, feeding a fresh id per render into `flash`'s hook root. The group takes an `id` defaulting to `"flash-group"` and derives children as `"<id>-<type>"`, matching Phoenix's own `flash_group`.
